...
\title{IntroSpect-TS: Training-Free Data Quality Assessment \\
for Time Series Foundation Models via Behavioral Introspection}
...
\begin{abstract}
The quality of pretraining data fundamentally determines time series foundation model (TSFM) performance, yet the TSFM community lacks principled methods for assessing per-sample data quality. While the LLM community has developed sophisticated frameworks---LLM-as-judge scoring, multi-agent selection, introspection-guided approaches---these either cannot process numerical time series or require expensive auxiliary model training. We introduce IntroSpect-TS, a training-free method that extracts data quality signals directly from a frozen TSFM's inference behavior. The key insight: a pretrained TSFM implicitly encodes knowledge of normal temporal patterns; high-quality data elicits focused attention and smooth predictions, while noisy data provokes erratic behavior. IntroSpect-TS collects multi-signal behavioral traces during a single forward pass, then applies profile-conditioned peer calibration to isolate quality-induced deviations from domain-induced difficulty. We provide a formal justification of the calibration conditions (Proposition~1). Experiments across three TSFM architectures and standard benchmarks aim to demonstrate that IntroSpect-TS quality scores enable better data curation than raw-loss, statistical-heuristic, and learned baselines, with ablation confirming peer calibration as the critical component. The method requires no parameter updates, no LLM calls, and no auxiliary training, making it immediately applicable to any pretrained TSFM.
\end{abstract}

\section{Introduction}

Time series foundation models (TSFMs) have rapidly emerged as a dominant paradigm for forecasting, mirroring the trajectory of LLMs in NLP. Architectures such as TimesFM~\citep{das2024timesfm}, MOIRAI~\citep{woo2024moirai}, Chronos~\citep{ansari2024chronos}, MOMENT~\citep{goswami2024moment}, and Timer~\citep{liu2024timer} are pretrained on massive temporal corpora spanning hundreds of billions of time points across energy, traffic, weather, finance, and healthcare. The implicit assumption is straightforward: more data yields more generalizable models. However, this inclusion-by-availability paradigm neglects a fundamental question: is all pretraining data equally useful? In practice, large-scale corpora contain substantial amounts of noisy or redundant series---sensor malfunctions, missing segments, recording artifacts, or patterns so simple that they contribute negligible learning signal. Training on such data wastes compute and may degrade model performance through spurious correlations.

A natural response is to train a supervised quality classifier using known quality labels. However, TSFM pretraining corpora typically lack such labels---annotating even a modest fraction of 100B+ time points is infeasible. The challenge is therefore to assess quality \emph{without} labeled training data, using only signals already available from the pretrained model.

The LLM community has recognized analogous problems and developed a rich ecosystem of data quality methods~\citep{xie2023doremi, wettig2024qurating, bai2025multi}. Static filtering uses heuristic or classifier-based scores; online adaptive methods adjust sampling during training; and introspection-guided approaches such as IGDS~\citep{shi2026igds} extract causally-validated SAE features from LLM internals to select ``feature-resonant'' data with dramatic efficiency gains. However, all these methods either cannot process numerical time series directly---LLMs struggle with TS quality assessment as TSQAgent~\citep{wu2026tsqagent} recently demonstrated---or require expensive auxiliary model training. Notably, TSRating~\citep{chen2026tsrating} (ICLR 2026) uses LLMs as quality judges for time series but requires significant LLM API calls and a trained meta-learning scorer. The closest prior work on TS data quality~\citep{wen2024measuring} operates at the dataset level via contrastive accuracy. TSFMAudit~\citep{li2026tsfmaudit} employs probe-time dynamics for contamination auditing. The key gap we identify is that all existing approaches---whether LLM-based (TSRating, TSQAgent), contrastive (Wen et al.), or probe-based (TSFMAudit)---require either external evaluators, auxiliary training, or both. \textbf{No prior work achieves per-sample quality assessment using only the TSFM's own behavioral signals} without any external oracle or model training. This gap is both practically motivated (zero API cost, zero training overhead) and conceptually significant (the TSFM already knows what constitutes normal temporal patterns).

This paper focuses on a previously unaddressed question: \textbf{given a corpus of time series windows and a frozen pretrained TSFM, can we assess each window's training quality without auxiliary model training, parameter updates, or external evaluators?} Answering this question requires solving three interlocking sub-problems. First, what behavioral signals from the TSFM's forward pass carry information about input quality, and how to collect them efficiently? Second, how to disentangle genuinely low-quality data from data that is merely out-of-distribution---a clean financial series and a noisy electricity series can produce identical raw prediction errors? Third, how to validate that quality scores causally improve downstream training rather than merely correlating with convenient heuristics?

We propose IntroSpect-TS, a training-free behavioral introspection method addressing all three sub-problems within a unified four-phase framework. The core insight is that a pretrained TSFM, having been exposed to millions of time series, has implicitly learned to distinguish normal patterns from anomalous signals. Rather than attempting to make LLMs understand time series---an approach TSQAgent showed to be unreliable---we extract quality signals directly from the TSFM's own inference behavior. Phase~1 collects multi-signal traces (per-step errors, layer-wise attention entropy, inter-layer representation changes) during a single forward pass. Phase~2 resolves domain confounding through profile-conditioned peer calibration: using statistical profiles to group structurally similar series, we compute quality scores as normalized behavioral deviations from domain-matched peers. Phase~3 stratifies the corpus by quality, and Phase~4 validates scores through downstream fine-tuning. We provide a formal justification for the calibration conditions (Proposition~\ref{prop:identification}), making explicit the assumptions required for valid quality identification. Importantly, IntroSpect-TS requires no auxiliary model training, no LLM API calls, and no TSFM parameter updates---it is immediately deployable on any pretrained TSFM.

Our contributions are:
\begin{enumerate}
    \item We formalize training-free data quality assessment for TSFMs, proposing IntroSpect-TS---the first behavioral introspection method for this domain. The framework decomposes the problem into signal extraction, domain-quality disentanglement, and causal validation.
    \item We provide a formal justification (Proposition~\ref{prop:identification}) for profile-conditioned peer calibration, establishing the assumptions under which peer-normalized behavioral deviations isolate quality from domain difficulty, with a practical algorithm using robust median/IQR statistics.
    \item Through a systematic experiment plan spanning three TSFM architectures, seven benchmarks, ablation, a controlled hard-vs-dirty test, and comparison against both heuristic and learned quality scoring baselines, we aim to validate that TSFM behavioral signals provide actionable quality information beyond what static profiles or raw losses capture.
\end{enumerate}
...
\midrule
\textbf{IntroSpect-TS} & \textbf{Training-free quality via behavioral introspection} & \textbf{---} \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Method: IntroSpect-TS}

\begin{algorithm}[t]
\caption{IntroSpect-TS}
...
\begin{table}[t]
\centering
\caption{Key challenges and IntroSpect-TS design choices.}
\label{tab:challenge_module}
\begin{tabular}{p{3.5cm}p{5.5cm}p{5.2cm}}
\toprule
\textbf{Challenge} & \textbf{Why It Is Hard} & \textbf{IntroSpect-TS Approach} \\
\midrule
...
\end{equation}
Uniform weights ($w_d = 1/D_B$) follow from the principle of maximum entropy when no prior information about dimension importance is available, and serve as the zero-shot default. We emphasize that the algorithmic simplicity of IntroSpect-TS---KNN over profiles, $z$-score normalization, weighted sum---is an intentional design choice, not a limitation. The method's purpose is to extract quality signals that already exist within the TSFM, not to learn new representations. This mirrors the philosophy of non-parametric statistics: when the signal is present in the data, a simple estimator with the right conditioning structure suffices. The supervised learning baseline in \S\ref{sec:learned_baseline} tests whether additional model capacity improves upon this simplicity.
...
\begin{figure}[t]
\centering
\fbox{\parbox{0.9\textwidth}{\vspace{1.5cm}\centering [Figure 1: IntroSpect-TS framework. \textbf{Phase 1} (left): A single TSFM forward pass extracts four behavioral signal categories---per-step prediction error trajectory, layer-wise attention entropy, inter-layer representation change, and overall loss. \textbf{Phase 2} (center): Statistical profiles group structurally similar series via KNN; per-sample behavior is $z$-score normalized within peer groups to produce calibrated quality scores $q_i$. \textbf{Phase 3} (right, top): Scores stratify the corpus into quality tiers. \textbf{Phase 4} (right, bottom): Top-tier data is used to fine-tune the TSFM; downstream forecasting performance validates score quality.]\vspace{1.5cm}}}
\caption{Framework overview of IntroSpect-TS.}
\label{fig:framework}
\end{figure}

\subsection{Phase 4: Downstream TSFM Validation}

Quality scores are causally validated through a controlled data selection protocol. For a target TSFM, we fine-tune on subsets selected by different criteria---IntroSpect-TS top-$\alpha$, bottom-$\alpha$, random-$\alpha$, raw-loss-$\alpha$, SNR-$\alpha$, anomaly-rate-$\alpha$, learned-$\alpha$, and full data---measuring downstream forecasting MSE and MAE across multiple prediction horizons ($H \in \{24, 96, 336\}$). The validation addresses SP3 through three comparisons:

\textbf{Discriminability.} The performance gap between top-$\alpha$ and bottom-$\alpha$ subsets quantifies whether quality scores carry genuine signal. A large gap (top $\gg$ bottom) confirms that IntroSpect-TS distinguishes informative data from detrimental data. Random-$\alpha$ serves as a calibration baseline: top-$\alpha$ should outperform random-$\alpha$, and bottom-$\alpha$ should underperform it.

\textbf{Calibration value.} Comparison between IntroSpect-TS top-$\alpha$ and raw-loss-$\alpha$ isolates the contribution of peer calibration. If profile-conditioned calibration resolves domain confounding as Proposition~\ref{prop:identification} claims, top-$\alpha$ should outperform raw-loss-$\alpha$, which selects data based purely on uncalibrated prediction error.
...
\textbf{Baselines.} (1) Random selection; (2) Raw Loss (lowest $L_i$); (3) SNR-based filtering (highest signal-to-noise ratio); (4) Anomaly-rate-based filtering (lowest IQR-outlier proportion); (5) LLM Direct Score---GPT-4o prompted with ``Rate the quality of this time series data on a scale of 1--5 for training a forecasting model. Consider noise level, pattern regularity, and presence of anomalies. Output only the integer score.'' followed by the raw numerical values; (6) \textbf{Learned Scorer}---an XGBoost classifier trained on profile features $\mathbf{p}_i$ using labels derived from held-out TSFM fine-tuning outcomes (top vs.\ bottom 25\% by MSE on a validation split). This baseline uses the same downstream evaluation protocol as Phase~4, requiring a one-time fine-tuning pass to produce labels; once trained, the Learned Scorer can score new data without further TSFM training. It directly tests our central claim: if profile features alone, with supervised training, can match IntroSpect-TS's training-free quality scores, then behavioral signals add no value. We note this baseline is not training-free---it deliberately tests the supervised upper bound that IntroSpect-TS must approach to justify its training-free design. (7) Full data (upper bound). We do not directly compare against TSRating~\citep{chen2026tsrating} as it requires LLM API access and scorer training that are orthogonal to our training-free constraint; a qualitative comparison is provided in the Discussion.
...
Table~\ref{tab:main_results} presents the projected validation. Two primary patterns are anticipated. First, IntroSpect-TS Top-25\% consistently outperforms Bottom-25\% with a substantial margin across all model-dataset pairs, confirming discriminative power. Second, Top-25\% outperforms Raw Loss, demonstrating that profile-conditioned calibration extracts quality signals beyond what raw prediction error provides. The SNR and anomaly-rate baselines are expected to underperform IntroSpect-TS because they capture only single quality dimensions (noise level, outlier density) without the multi-signal behavioral richness or domain calibration.

\begin{table}[t]
\centering
\caption{Projected main results (MSE $\downarrow$, $\dagger$ = projection). Columns: Top/Btm = IntroSpect-TS top/bottom 25\%; Rnd = Random; Loss = Raw Loss; SNR = SNR-based; Anom = Anomaly-rate; Lrn = Learned Scorer (XGBoost); Full = all data. Bold: best selection method.}
...
Table~\ref{tab:main_results} (Learned column) includes an XGBoost classifier trained on profile features $\mathbf{p}_i$ with binary labels from TSFM fine-tuning outcomes. This baseline directly addresses whether behavioral signals provide information \emph{beyond} what profile features alone can capture through supervised learning. The Learned Scorer is projected to outperform statistical heuristics (SNR, anomaly rate) but underperform IntroSpect-TS. If this projection holds, it confirms that TSFM behavioral signals contain quality-relevant information not present in static profile features. If the Learned Scorer matches or exceeds IntroSpect-TS, the central claim---that behavioral introspection adds value over profile-based alternatives---would be falsified.
...
\midrule
Full IntroSpect-TS & 0.053$^\dagger$ \\
...
As shown in Table~\ref{tab:hard_vs_dirty}, raw prediction loss is projected nearly identical for both sets. IntroSpect-TS with peer calibration sharply separates them (HBC $\gg$ EBD). Crucially, removing calibration collapses the separation to near zero, providing direct causal evidence for Proposition~\ref{prop:identification}. Statistical heuristics (SNR, anomaly rate) correctly rank HBC above EBD but produce coarser binary signals without the multi-dimensional behavioral nuance.
...
IntroSpect-TS $q_i$ & +0.42$^\dagger$ & --0.67$^\dagger$ & \textbf{Yes (large gap)} \\
...
\textbf{Selection ratio sweep.} We evaluate $\alpha \in \{0.1, 0.25, 0.5, 0.75, 1.0\}$ to assess consistency across data budgets. IntroSpect-TS top-$\alpha$ is expected to maintain advantage over baselines across all ratios, with the largest relative gain at low $\alpha$.

\section{Discussion}

\subsection{Why Behavioral Introspection Works}

Beyond the formal identification argument (Proposition~\ref{prop:identification}), IntroSpect-TS's effectiveness can be understood through two broader perspectives. \textbf{First, the method exploits an information asymmetry:} the TSFM has access to richer signals about input quality than any external evaluator. A pretrained TSFM has already internalized the statistical regularities of its training distribution; quality assessment reduces to asking ``does this sample fit the learned distribution?''---a question the TSFM answers implicitly through every forward pass, without additional training. \textbf{Second, peer calibration can be viewed as a non-parametric implementation of the backdoor criterion in causal inference:} $\mathbf{p}_i$ blocks the backdoor path $Q_i \leftarrow \text{domain} \rightarrow \mathbf{B}_i$, enabling identification of the direct effect of $Q_i$ on behavior within each profile stratum. This causal framing clarifies why profile choice matters: profiles must block backdoor paths without creating new ones, motivating the deliberate selection of domain-related structural features that are orthogonal to per-sample quality variation.

\subsection{Relationship to IGDS and Feature Introspection}

IGDS~\citep{shi2026igds} and IntroSpect-TS converge on ``model-internal signals guide data'' through complementary mechanisms. IGDS requires SAE training for targeted feature extraction; IntroSpect-TS uses raw behavior for zero-cost deployment. TSRating~\citep{chen2026tsrating} and IntroSpect-TS address the same problem (TS data quality) but through orthogonal oracles: TSRating relies on LLM judgments distilled into a meta-learned scorer, while IntroSpect-TS extracts signals from the TSFM itself without any external evaluator or training. Together, IGDS, TSRating, and IntroSpect-TS span the precision-cost frontier of introspective data quality methods.
