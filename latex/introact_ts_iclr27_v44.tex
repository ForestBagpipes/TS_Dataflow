...
historical evidence is insufficient; abstention returns the untouched reference forecast
and is treated as a valid outcome rather than a failure. Across \ph{N-DATASETS} datasets,
four missingness patterns, and two forecasting horizons, using two frozen \tsfm{}
backbones for development and a third family held out from method design,
\introact{} achieves \ph{RESULT-MAIN} while reducing harmful interventions by
\ph{RESULT-HARMFUL} relative to \ph{STRONG-BASELINE}, at an intervention rate of
\ph{RESULT-INTERVENTION-RATE}. These results suggest that incomplete-input handling can
benefit from an explicit decision about when \emph{not} to act, rather than treating
repair as a mandatory preprocessing step.
\end{abstract}
...
Three observations motivate the design. \textbf{(i)}~Reconstruction improvement and
forecasting utility are not equivalent: an intervention that reduces reconstruction error
can still degrade the frozen forecaster's output. \textbf{(ii)}~The best intervention
varies across episodes and backbones. \textbf{(iii)}~A wrong intervention can be worse than
leaving the input unchanged, so unconditional ``repair'' is not merely suboptimal but
actively harmful on a non-trivial fraction of requests. Together these turn incomplete-input
handling into a decision problem rather than a preprocessing problem.
...
predictions; the returned forecast always comes from exactly one actually executed input
version.
...
\begin{figure}[t]
  ...
  the deployment missingness mechanism is injected and every legal action is actually
...
one actually executed input version, so no output fusion or post-hoc correction can
...
formalises when information is worth its cost; our selection rule stays a
...
\subsection{Selective Decisions and Off-Policy Evaluation}
\label{sec:related-ope}
...
jiang2016doubly,thomas2016data}, but three differences matter here. First, an arm's value
is not a designed reward but the realised loss of a frozen, non-differentiable forecaster,
so there is no reward model to fit and no simulator to query: the only legal supervision is
a replayed historical window, tied to the exact model revision that produced it. Second,
abstention is a first-class action with a known-safe outcome, so a wrong decision is
asymmetric and directly measurable --- which is why we report harmful intervention rate and
harmful loss rather than regret alone. Third, we explore nothing and fit no policy
parameters; the selector is a fixed local-average rule over retrieved replays, which keeps
it auditable but also caps its ceiling. Selective prediction and learning to
defer~\citep{chow1970optimum,vovk2005algorithmic,geifman2017selective,madras2018predict,
mozannar2020consistent} are the closest relatives, but they decide whether to
\emph{predict} or hand the case to a human or a second model; we decide whether to
\emph{modify the input} of one fixed predictor, and our abstention returns that
predictor's own reference forecast.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.72\textwidth]{figure/fig2_introact_architecture.pdf}
  ...
  forecast always comes from a single actually executed input version.}
  \label{fig:arch}
\end{figure}
...
sense: $g_{i,a}$ is the outcome of an action the deployed system did \emph{not} take,
recovered by replaying a historical window rather than by modelling it. No unconfoundedness
assumption is invoked, because both $F(X_i^{a_0})$ and $F(X_i^{a})$ are evaluated on the
same fully observed window, so $g_{i,a}$ is a realised measurement rather than an estimate
of a latent potential outcome.
...
therefore be replayed as a \emph{pseudo-deployment}.
...
\S\ref{sec:exp-setup}), obtaining an incomplete input $(X_i, M_i)$, and then actually
...
Figure~\ref{fig:concept}(b) summarises this construction. The bank is a discrete grid
rather than a model of missingness; the resulting scope of validity is discussed in
...
the candidate intervention would change}. The state uses no learned encoder, only
...
A request is served in six steps: read the visible context and mask; execute the reference
action to obtain $p_0$; materialise the legal candidates; extract $z$; query the frozen bank
and compute $S_a$ for every candidate; then abstain if $\max_a S_a \le 0$, otherwise
execute $a^\star$ and return $F(X^{a^\star})$. The backbone is called once for the
reference and once for the selected action, so a request costs one or two \tsfm{} forecasts
(\S\ref{sec:exp-efficiency}). There is no ensemble, no output correction, no residual
model, and no parameter update. Cost accounting is kept explicit: candidate
materialisation, imputer runtime, both \tsfm{} calls, selector overhead, and total request
wall-clock are recorded separately, and no component is booked as free because a cached
artefact happened to exist.
...
with \textsc{Keep} as the reference $a_0$. The catalog stays at five arms because the
contribution under test is the selection rule, not imputer architecture; a wider catalog
would confound the two. A complete and
...
used for evaluation but not to refit the selector. Replay pseudo-deployment severities are
drawn from the same three levels by deterministic hash, so the method is a single
configuration that is not retuned per severity.
...
Cost is reported separately in Appendix~\ref{app:calls} and is never folded into an
accuracy score. Offline training/search is the one-off cost of fitting trainable imputers
or running a transformation search; online cost covers candidate construction, frozen-model
calls, and end-to-end request latency. Cached historical artefacts are not booked as zero,
and batch-averaged latency is not presented as a per-request guarantee; cold start is
...
Table~\ref{tab:app-recutils} (Appendix~\ref{app:recutils}).

\begin{figure}[t]
  \centering
  \includegraphics[width=0.60\textwidth]{figure/fig3_reconstruction_vs_utility.pdf}
...
The claim this experiment supports is narrow: \emph{reconstruction quality is not a
reliable proxy for frozen-\tsfm{} forecasting utility}. If the measured correlation
...
The same five published baselines enter as in \S\ref{sec:exp-main}. We report four governance diagnostics: the
...
unconditionally and conditional on an intervention; the
\emph{harmful loss}
...
\begin{equation}
  ...
  \label{eq:cgc}
\end{equation}
computed only on improvable windows where the oracle strictly beats \textsc{Keep}. The name
reflects that the baselines can produce inputs outside the five-arm catalog; it is not a
universal oracle gap. Figure~\ref{fig:governance} in Appendix~\ref{app:governance-fig}
shows the per-method harmful loss and the realised utility against the binned score.
...
The same five published baselines enter, with no method retuned between severities.
Severities are $10\%$, $30\%$ and $50\%$, crossed with
...
\emph{Which component carries the gain, and is local historical replay actually
necessary?}
...
\begin{table}[t]
  \caption{Ablation on the main protocol; $\checkmark$ marks a present component. Calls is
  the mean number of frozen-model forecasts per request.}
  \label{tab:ablation}
  \centering
  \small
  \setlength{\tabcolsep}{4pt}
...
untouched, returns a forecast from exactly one actually executed input version, and can
...
for a specific piece of evidence does not have to scan the whole appendix.
...
\resizebox{\textwidth}{!}{\begin{tabular}{lll}
    ...
    \midrule
    A. Protocol details   & splits and purging, action catalog, observable features, missingness patterns, replay-bank scope & what exactly was fixed before the experiment ran? \\
    B. Full per-source results & every source, backbone, horizon and metric at $10\%$ severity & does the aggregate hold source by source? \\
    C. Full severity tables & $10/30/50\%$ for every method and metric & does the frozen policy degrade slower than the baselines? \\
    D. Per-pattern results & P1--P4 reported separately & which missingness structure drives the result? \\
    E. Reconstruction vs.\ utility & full paired table and discordant rate & is reconstruction a usable proxy? \\
    F. Governance diagnostics & harmful loss and score-sign agreement & does abstention remove harm? \\
    G. Robustness figure & severity trend per backbone & how does the trend look, not just the endpoints? \\
    H. Replay size & $25/50/100\%$ of the bank & how much history does the method need? \\
    I. $K$ and $\beta$ sensitivity & the full $3\times3$ gate grid & is the frozen choice robust or arbitrary? \\
    J. Call accounting & latency, call counts, failures, timeouts & what does a request actually cost? \\
    K. Preceding-cycle negative result & the failed projection attempt & what was tried and rejected, and why? \\
    L. Complete-input contract & required behaviour by input regime & what happens when nothing is missing? \\
    M. Finance case study & two finance parents, QLIKE & what happens outside the benchmark sources? \\
    N. Ten mask seeds & mask-realisation stability & is the conclusion an artefact of one mask? \\
    O. Additional horizons & horizons outside $\{96,192\}$ & does the conclusion extend further out? \\
    P. Reproducibility details & record schema and result groups & can every number be traced to an artefact? \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\subsection{Scope of Validity of the Replay Bank}
\label{app:scope}
...
family --- the matcher still returns nearest neighbours, but the retrieved $g_{i,a}$ need
not transfer, and the conservative gate is the only protection against that mismatch. We
therefore read \S\ref{sec:exp-robust} as a within-grid check, not as out-of-grid
generalisation, and the finance case study (Appendix~\ref{app:finance}) is the only contact
with a non-synthetic gap in this paper.
...
\label{app:features}
The task state $z_i$ is lightweight and fully observable at request time. It
...
behind the summary in \S\ref{sec:exp-recutility}. Reconstruction error uses the artificially
...
Table~\ref{tab:governance}. Panel~(b) is a sign-agreement check rather than a calibration
curve: the conservative score is used only to order actions and
