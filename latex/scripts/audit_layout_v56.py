...
baseline=(HERE.parent/'archive/paper_20260923/pre_language'/ (NAME+'.tex')).read_text(encoding='utf8')
...
for label in old:
    if label=='tab:app-holm':
        continue  # Replaced the outdated five-comparison table from saved nine-comparison records.
...
report={'manuscript':NAME,'main_text_last_page':page,'pdf_pages':len(PdfReader(HERE/(NAME+'.pdf')).pages),
        'tables_with_numeric_cells_preserved':len(new)-1,
        ...
        'new_experiments':False,'scope':'Layout and stored-value preservation, with the obsolete Holm table updated from saved v55 statistics. No new citation audit or experimental rerun.'}
...
assert len(re.findall(r'^## 对照 \d+',translation,re.M))==294
assert len(re.findall(r'^### 共用数值表 \d+',translation,re.M))==50
report['bilingual_text_blocks']=294
