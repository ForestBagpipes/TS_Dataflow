...
Neighbourhood/Local only 为仅局部邻域，Source mean only 为仅数据源均值，No retrieval 为无检索，Linear utility model 为线性效用模型，No action conditioning 为无动作条件，Always repair 为始终修复，No intervention state 为无干预状态，No reference forecast 为无参考预测特征，Full/FULL 为完整方法。Mean only 为仅均值，Linear 为线性，Random 为随机。identical by construction 为按构造相同。

P1 Point 为点缺失，P2 Target Block 为目标块缺失，P3 Cross-Channel Block 为跨通道块缺失，P4 Tail 为尾部缺失，Avg. Rank 为平均排名。all eight 为全部八源，without Exchange 为去除 Exchange，vs 为相对于，fixed 为固定策略。No-op 为不操作，Low/High opportunity 为低/高机会，Stratum 为层，Boundary factor 为边界倍数，Share of episodes 为样例占比，Median 为中位数，90th pct. 为第 90 百分位。
...
历史结果要能指导新请求，需要解决两个困难。**（1）相关证据可能稀缺。** 相近请求可能很少，而数据源整体均值无法区分同一数据源内的请求。效用估计需要同时考虑历史证据的相关性和数量。**（2）平均精度不能描述修复造成的伤害。** 修复可能降低平均误差，同时使个别预测变差。决策需要根据可观测信息判断是否执行推荐，并分别评价其精度和伤害，不能访问该请求的未来。
...
**（1）面向预测的修复选择。** 我们提出 IntroAct-TS，结合每种修复的局部预测效用估计和数据源层面的估计，再相对于保持输入不变决定是否执行修复。**（2）具有配对结果度量的决策问题。** 我们在冻结预测器的条件下形式化选择性修复，并定义效用与伤害损失，以区分预测改善和修复造成的退化。历史全动作回放无需使用新请求的未来即可提供监督。**（3）跨数据源和骨干的证据。** 我们在八个数据源和三个骨干上与九种可部署对照比较，并在另外两个数据源上评估。结果支持相对于保持输入不变的改善，并揭示精度与伤害之间的权衡，相对于固定策略的差异则取决于骨干。
...
**回放使预测效用可观测。** 对每个已完成的 TRAIN 窗口，我们生成登记的缺失模式，构造每种可用修复，并用同一已实现未来评价各修复输入。式 \eqref{eq:utility} 随后为每个动作提供配对效用标签。每个标签与下述状态一同存储。
...
状态使用可见请求、候选输入和参考预测，不使用未选中候选的预测。库 $\calB=\{(z_{i,a},a,g_{i,a})\}$ 从 TRAIN 一次构建，部署时保持只读。每个冻结骨干版本具有独立的库，覆盖登记的缺失网格。附录 \ref{app:replay} 给出构建细节。检索假设相近的动作条件状态具有相似效用，第 \ref{sec:exp-ablation} 节的估计对照检验这一假设。
...
**局部样例区分同一数据源内的请求。** 22 维状态描述缺失模式、可见上下文、候选变化和参考预测。候选变化相对于可见目标的插值计算，不使用隐藏值。特征在库上标准化，检索可以跨数据源和预测长度进行。附录 \ref{app:features} 定义各特征。
...
**数据源均值稳定稀疏的局部证据。** 邻域独立证据较少时，局部均值可能波动。库还记录同一动作在该请求所属数据源上的表现。数据源估计使用更多历史，但不能区分同一数据源内的请求。以 $m_{s,a}$ 表示该数据源可用回放记录的平均效用，与 Source Fixed 对照所用统计量一致。估计器结合局部估计和数据源估计。
...
**执行需要正的惩罚后估计。** 邻域可能很小或内部不一致，因此仅有正均值可能不可靠。决策据估计所依赖证据的稀缺程度，对离散程度施加相应惩罚。
...
**最合适的修复及其价值随请求变化。** 图 \ref{fig:utility}(a,b) 报告的 oracle 最优占比为 11.1% 至 24.8%，KEEP 在 19.3% 的样例上最优。oracle 的 MASE 为 1.258，KEEP 为 1.576，说明该动作目录提供了选择机会。此 oracle 使用未来结果。在 Bolt 上，重建与预测的最优动作在 19.3% 的样例上一致，成对排序不一致率为 44.5%，样例内平均相关系数为 0.12。这些诊断支持将预测效用用作监督，但不说明重建信息在所有情形下均无帮助。附录 \ref{app:recutils} 报告排序，图 \ref{fig:cases} 给出回顾性案例。
...
**选择性修复改善总体预测。** 表 \ref{tab:main} 在 10% 缺失率下比较各方法。IntroAct-TS 的数据源宏平均 MASE 为 1.425，在可部署方法中总体最低。KEEP 为 1.576，Best Fixed 为 1.488，Source Fixed 为 1.445。R2-CART 和 Fixed SAITS 均为 1.459。完整方法修复 55.2% 的请求。这些均值支持选择性修复，不过各骨干上的最佳方法不同。
...
**较低修复频率伴随较低伤害损失。** 伤害损失度量修复引起的误差增加，并在全部请求上平均。图 \ref{fig:harm}(c,d) 中，IntroAct-TS 的伤害损失为 0.0340，修复率为 55.2%。Best Fixed 分别为 0.0513 和 74.5%，R2-CART 分别为 0.0552 和 98.7%。这些结果显示较低总体伤害，但干预频率也有差异。
...
为评价请求信息的作用，随机对照保留完整规则推荐的动作，只随机决定是否执行。其概率匹配 FULL 的开发块修复率，并在 TEST 上冻结。三个骨干上 FULL 减随机对照的 MASE 差值为 −0.0534、−0.0142、−0.1126，区间分别为 [−0.0751,−0.0336]、[−0.0281,−0.0001]、[−0.1818,−0.0448]。这些名义区间均不含零。比较支持利用信息的执行判据优于该随机对照。实际 TEST 频率不同，因此并未确立严格相同干预率下的优势。
...
**选择性执行以平均精度换取较低伤害。** 去除这一机制并修复每个请求后，MASE 从 1.425 降至 1.414，伤害损失则从 0.0340 升至 0.0515。因此点估计支持始终修复，而执行判据将额外伤害降低约三分之一。第 \ref{sec:exp-harm} 节给出这一权衡的校准工作点。
...
**输入修复应按其预测效果评价。** IntroAct-TS 利用历史动作结果，为每个不完整请求选择修复，同时保持预测器固定。在八个数据源和三个骨干上，其总体 MASE 为 1.425，保持输入不变为 1.576，相对于固定策略的结果则依赖骨干。选择性执行减少伤害损失，代价是平均误差高于始终修复。额外两源评估支持在两个骨干上优于 KEEP，但未确立相对于 Best Fixed 的一致优势。这些发现支持同时报告精度和修复造成的伤害。由于早期设计使用过同一评价块，主要 TEST 比较仍属于探索性结果。
...
附录先给出实验证据，再给出复现和解释所需设置。**（1）结果与敏感性**涵盖预测、执行对照、消融、三个严重度及额外两源评估。**（2）方法与评价细节**定义数据划分、动作、特征、选择和校准。**（3）支持性记录**说明成本、可复现性、历史分析和相关工作。历史结果与当前配置明确分开。
...
本节从三方面展开主要预测比较。**（1）** 动作层面的效用和重建排序解释为何需要选择修复。**（2）** 八个数据源、两个预测长度和三个骨干的结果说明总体结果的来源。**（3）** 数据源固定策略和留一数据源比较检验增益是否依赖数据源身份。
...
本节考察请求信息是否改善执行决策。**（1）** 三种替代方案分别使用仅均值、线性或随机决策，并在开发侧匹配执行频率。**（2）** 比较报告三个骨干各自的预测误差、执行频率和伤害损失。历史工作点及机会分析单独保留在附录 \ref{app:history}。
...
表 \ref{tab:v52-gates} 在 TEST 上比较冻结执行判据与事后对照。这些对照在查看早期 TEST 结果后运行，使用保留的预测缓存和完整方法冻结设置，不构成独立确认集。本节所有区间均为记录的 2,000 次配对父窗口自助法区间，种子 101，为名义区间，未作多重比较校正。
...
TEST 上的执行判据对照。阈值和随机接受概率在开发块校准。HIR 以已发生干预为条件。
...
随机对照使用单个已记录种子 101。其区间重采样父窗口，不包含随机化种子之间的变动。仅均值和线性对照同时改变推荐和接受得分，因此这些比较不能单独分离惩罚的作用。三个冻结配置均使用 $\beta=1.64$。仅均值对照将其设为零，并在 TRAIN 上匹配阈值。不使用任何 TEST 结果重新匹配实际干预频率。
...
本节区分估计器的三个问题。**（1）** 固定配置消融分离检索、汇聚和执行变化的作用。**（2）** 两个特征组对照检验状态表示。**（3）** 最后一小节说明哪些缩减状态选择实验尚未纳入当前评价。
...
表 \ref{tab:v52-a2} 报告六个变体的分骨干结果及配对差值。差值为 FULL 减该变体，负值有利于 FULL。两行改变汇聚端点，一行取消检索，两行分别去掉两个特征组，一行取消执行判据。
...
三项检查说明已记录结果的范围。**（1）** 严重度扫描评价 10%、30% 和 50% 缺失率。**（2）** 库子采样保留 25%、50% 和 100% 的历史父窗口。**（3）** 两个额外掩码实现在相同父窗口上重复评价。所有检查均保持所选配置。
...
本节报告额外两个数据源上的评价。**（1）** 我们说明其开发历史，并区分当前 TRAIN 和 TEST 的作用。**（2）** 在 21 个测试父窗口上，将冻结流程与五种可部署对照比较。这些结果检验流程迁移，而非向完全未见领域的迁移。
...
本节从三方面规定实验设置。**（1）** 范围、时间划分和四种缺失模式定义请求。**（2）** 指标与计数约定固定单位和分母。**（3）** 基线可用性和配置说明各比较所评价的内容。
...
列为模式、删除内容、纳入原因。P1 点缺失删除分散单点，对应经典随机缺失。P2 目标块删除目标变量连续块，对应被预测变量的传感器故障。P3 跨通道块删除同一时段的多个可见变量，对应影响整个面板的通道或采集故障。P4 尾部缺失删除紧邻预测起点的块，对应请求时尚未到达的观测。
...
本节给出解释方法所需的实现细节。**（1）** 六种目录动作定义允许的输入变化。**（2）** 四组共 22 个特征及回放构建定义历史证据。**（3）** 决策设置和完整输入要求限定方法可以支持的主张。
...
选择性预测以覆盖率换取较低风险，学习转交则将请求路由至另一决策者。IntroAct-TS 始终返回同一模型的预测，并决定是否修复其输入。这属于以修复效用监督的逐实例算法选择。历史回放评价每个可用动作，提供全动作反馈，而离策略评价仅能获得已记录动作的结果。附录 \ref{app:ope-positioning} 详述这一关系。
