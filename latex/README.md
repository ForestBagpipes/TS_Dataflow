...
v45 共 **1322** 个唯一 KEY。完整清单见 `docs/v45_placeholder_keys.csv`
（两列 `key,family`），实验侧的执行规划见
`docs/IntroActTS_20260918_v45_experiment_plan.md` §10。

| family | 数量 | 键模式 |
|---|---|---|
| `main_table_cell_per_source` | 540 | `SRC_<BACK>_<METHOD>_<H>_<SRC>`，`<BACK>` ∈ `BOLT/TF/CH2`，`<METHOD>` ∈ `KEEP/BF/R2/TOI/TOIVSF/GIMCC/SRDI/CTF/OURS/ORACLE`，`<H>` ∈ `96/192`，`<SRC>` ∈ `ETTH1/ETTH2/ETTM1/ETTM2/ELEC/EXCH/TRAF/WEA` |
| `main_table_macro` | 60 | `SRC_<BACK>_<METHOD>_<H>_MACRO` |
| `per_method_appendix` | 152 | `<METHOD>_<BOLT/TF/CH2/OVR/RMSSE/RANK/CAND/CALLS/MEAN/MAX/P95/ON*/OFF*/S25_*>` |
| `appendix_severity_table` | 139 | `SEV<10/30/50>-<METHOD>-<MASE/RMSE/MAE/MSE/RANK>` |
| `table2_harm` | 61 | `HARM_<IR/HIR/HL/BP/MO/MASE>[_<METHOD>]`、`HARM_RISKCURVE`、`HARM_READING` |
| `appendix_ablation` | 55 | `AF_<FULL/A1..A5/ORACLE>_<MASE/RMSE/MAE/MSE/CHIR/HL/IR/CALLS>` |
| `table4_ablation` | 53 | `ABL_<FULL/A1..A5/ORACLE>_<MASE/HIR/HL/IR/CALLS/LAT>`、`ABL_RANKFULL_*`、`ABL_STAB_*` |
| `table3_robustness` | 39 | `RB_<METHOD>_<10/30/50/CH2/WORST>`、`ROBUST_*` |
| `appendix_per_pattern` | 36 | `P<1..4>_<METHOD>` |
| `appendix_replay_size` | 33 | `SUPPORT<25/50/100>_<METHOD>`、`RS<25/50/100>_*` |
| `appendix_operating_points` | 27 | `NOOP_*`、`LOWOP_*`、`HIGHOP_*` |
| `appendix_mask_stability` | 25 | `SEED1..6_*`、`SEED_SPREAD_*`、`SEED_IR_SPREAD_*`、`SEED_MASK/ORDER/SELECTOR` |
| `appendix_extended_comparison` | 25 | `OVR_<METHOD>`、`PR_<METHOD>`、`OFFLINE_<METHOD>` |
| `appendix_reconstruction_oracle` | 24 | `REC_<METHOD>_<MASE/MAE/MSE/GAIN>` |
| `narrative_sentence` | 20 | `MAIN_READING`、`HARM_READING`、`ABL_READING`、`ROBUST_READING`、`RANK_DISAGREE_STATS`、`DISCORDANT_RATE`、`CONCLUSION_CLOSING`、`CONCL_*`、`ABSTRACT_CLOSING`、`COLDSTART` |
| `appendix_protocol_counts` | 18 | `TEST_*`、`FIT_*`、`GATE_*`、`EVAL_*`、`N_DATASETS`、`N_BOOTSTRAP`、`FAILRATE`、`RECON_ORACLE_MASE` |
| `appendix_cost` | 17 | `CALLS_*`、`COST_*` |
| `supplementary_finance` | 15 | `FIN-*`、`FIN_P1`、`FIN_P2` |
| `appendix_gate_grid` | 12 | `OPPSENS_*` |
| `appendix_opportunity_strata` | 12 | `STRATUM_*` |
| `appendix_reproducibility` | 11 | `REV_*`、`PYVERSION`、`NUMPYVERSION`、`PANDASVERSION`、`SKLEARNVERSION`、`TORCHVERSION`、`HARDWARE`、`PRECISION`、`PROTOCOL_COMMIT` |
| `appendix_baseline_roster` | 5 | `VIDA_*` |
| `appendix_rank_disagreement` | 3 | `RHO_BOLT/_TF/_CH2` |
...
5. **不要新增未列出的 KEY**。若确实需要，先更新本文 §3.1 与
   `docs/v45_placeholder_keys.csv`，再在 `.tex` 里使用。
...
补遗里的 Development Negative Results 与 Financial Case Study 已压缩。
若金融 case study 保留了 verified natural-missingness 结果，必须保留
"case study only，不可与主表比较"的显式声明，不得删掉这句。
...
| 附录 | 内容 |
|---|---|
| A | Protocol、最终 TEST split、action catalog、features、missingness、metric conventions |
| B | full per-source / per-pattern 结果 |
| C | robustness core set 的 full severity 结果 |
| D | reconstruction / utility 诊断与 Reconstruction Oracle |
| E | full ablations、replay size、$k/\beta$ gate grid、3-mask stability |
