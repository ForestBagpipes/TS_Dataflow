...
The action catalog offers useful choices because both the preferred repair and the value of
repairing vary by request. In Figure~\ref{fig:utility}(a,b), oracle-best shares range from
$11.1\%$ to $24.8\%$, and KEEP is best on $19.3\%$ of episodes. The catalog oracle reaches
1.258 MASE versus 1.576 for KEEP. This gap measures the opportunity available to a selector,
although the oracle uses future outcomes and cannot be deployed.

Reconstruction alone does not identify the best forecasting action reliably in these records.
On Bolt, the reconstruction and forecasting winners agree on $19.3\%$ of episodes, with
$44.5\%$ pairwise ranking disagreement and mean within-episode Spearman correlation 0.12.
These diagnostics motivate using forecasting utility as supervision. They do not rule out
all selectors that use reconstruction features. Appendix~\ref{app:recutils} reports the
rankings, and Figure~\ref{fig:cases} gives two retrospectively selected examples.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figure/fig8_diagnostics.eps}
\caption{Repair utility, decision harm and sensitivity under frozen configurations.
(a) Filled bars show oracle-best action frequencies and hatched bars the share improved by
that action. (b) Mean utility relative to KEEP, with positive values indicating lower MASE.
Beneficial shares and utilities use only episodes on which the action is admissible.
Action codes in (a,b) are K: KEEP, F: forward-fill, S/M: single/multivariate TS-ICL,
R: context ridge, and A: SAITS. (c) Harmful loss averages repair-induced increases in error
over all requests. (d) Filled bars show intervention frequency and hatched bars the harmful
share among executed interventions. KEEP's conditional harmful rate is undefined.
(e) Source-macro MASE at 10, 30 and 50\% missingness without retuning.
(f) Largest MASE increase over KEEP in any pattern, backbone and severity cell.
Method codes in (c,d,f) are K: KEEP, BF: Best Fixed, C: R2-CART, A: Fixed SAITS,
T: TATO, and IA: IntroAct-TS. Panels (a--d) use 10\% missingness. Method-level means
average three backbones, two horizons and four patterns with equal source weighting.
Bars and lines show recorded point estimates, not confidence intervals. All severities
occur in replay construction. Table~\ref{tab:heterogeneity} retains the action values. Appendix~\ref{app:governance-fig}
reports operating-point diagnostics, and Appendix~\ref{app:severity-full} gives the severity breakdown.}
...
R2-CART and Fixed SAITS each reach 1.459. TATO reaches 1.616 and TimesNet reaches
...
\label{sec:exp-harm}
Harmful loss measures the error added by detrimental repairs, averaged over all requests.
Figure~\ref{fig:harm}(c,d) shows 0.0340 for \introact{} at a $55.2\%$ repair rate, compared
with 0.0513 at $74.5\%$ for Best Fixed and 0.0552 at $98.7\%$ for R2-CART. TimesNet
repairs every request and records 0.1798. These observations connect the lower aggregate
harm to the decision to leave some inputs unchanged.

To examine whether request information helps beyond reducing repair frequency, a random
gate keeps the full rule's recommended action and matches its repair rate on the internal
evaluation block. Its probability is then frozen. FULL minus random MASE is $-0.0534$ on
Bolt, $-0.0142$ on TimesFM and $-0.1126$ on Chronos-2, with parent-clustered intervals
$[-0.0751,-0.0336]$, $[-0.0281,-0.0001]$ and $[-0.1818,-0.0448]$. All exclude zero.
Actual TEST repair rates differ, so these are development-calibrated comparisons.
...
Appendix~\ref{app:gate-controls} gives the values and calibration procedure.

Training-side threshold calibration meets the clipped-harm target in all four Chronos-2
cells and only at $\alpha=0.05$ on Bolt and TimesFM. Temporal blocks do not establish the
exchangeability assumption. Appendix~\ref{app:layout-details} retains all prospective
calibration observations and the retrospective TEST cross-fit table.
...
\begin{table}[t]
  ...
  all requests. Bold and underline mark the lowest and second-lowest error and harm values.}
  \label{tab:ablation}
  \centering
  \small
...
All three severities occur in replay construction. Appendix~\ref{app:layout-details} retains
the interpretation and mask-sensitivity checks, which reuse the same parent sets.
