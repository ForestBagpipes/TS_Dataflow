...
held out from method design, \introact{} lowers source-macro MASE from 1.576 to 1.423, against 1.438 for the best fixed intervention, improves on the untouched input on every backbone with paired intervals that exclude zero, and reaches the lowest macro average and the best average rank of the deployable rows on Bolt and TimesFM. On the held-out family one fixed repair reaches a lower average, which we report rather than average away. The evidence says that for a frozen forecaster the useful question about an incomplete input is which repair to run on this request, and that the answer can be read off what past repairs did to this forecaster.
\end{abstract}
...
\introact{} improves on the untouched input on every backbone with paired intervals that exclude zero, reaches the lowest source-macro MASE of the deployable rows on Bolt and TimesFM, and the lowest average over the three backbones at 1.423, at one to two backbone calls per request. We also report where it does not win (\S\ref{sec:exp-main}, \S\ref{sec:exp-harm}, \S\ref{sec:exp-robust}).
...
three backbones, two horizons, and four patterns. \introact{} reaches 1.423 source-macro MASE against 1.576 for the untouched input, 1.438 for the best fixed intervention and 1.456 for the simple selector, and against the published baselines at SAITS 1.696, TATO 1.689. It improves on the untouched input on every backbone.
Against the untouched input the paired difference is Bolt -0.1002 [-0.1530, -0.0496], TimesFM -0.1464 [-0.1985, -0.0932], Chronos-2 -0.2119 [-0.3053, -0.1203], and every interval excludes zero. Against the best fixed intervention the interval excludes zero on Bolt. It is the lowest deployable row on Bolt and TimesFM, and on Chronos-2 one fixed repair reaches a lower macro average, which \S\ref{sec:exp-robust} returns to. The deployable rows span 1.423 to 1.696 and the catalog oracle sits at 1.307, so the room to recover is bounded and the gaps between rows are a visible part of it. Both published baselines repair or transform every incomplete request, and SAITS ends at 1.696 and TATO ends at 1.689, above the 1.576 of leaving the input alone. SAITS is below it on 18 of 24 source and backbone cells, so what costs it its average is the minority of cells on which it is badly wrong, which is the failure a per-request rule can avoid. TATO is below it on only 5 of 24 such cells, so that one loses broadly. Against the simple selector no interval excludes zero, so accuracy alone does not separate the two and what does is in Table~\ref{tab:harm}.
Average rank over the evaluation cells is Bolt 4.05, TimesFM 3.99, Chronos-2 4.28, the best of the six deployable rows on Bolt and TimesFM, against 4.14 for the best fixed intervention on Chronos-2, and over the three backbones it averages 4.11 against 4.37 for the best fixed intervention. Holm correction over the five baseline comparisons leaves the difference against the untouched input below $0.05$ on every backbone and the one against the best fixed intervention below $0.05$ on Bolt.
...
\resizebox{\textwidth}{!}{\begin{tabular}{llccccc c}
    ...
    \midrule
    Native KEEP        & Frozen \tsfm{}          & 1.466   & 1.682   & 1.579   & 1.576   & 1.325   & 5.49 \\
    Best Fixed         & Fixed policy            & 1.386     & 1.576     & \best{1.353}     & \second{1.438}     & \second{1.233}     & \second{4.37} \\
    R2-CART            & Simple selector         & \second{1.375}     & \second{1.558}     & 1.435     & 1.456     & 1.244     & 4.85 \\
    SAITS              & Reconstruction          & 1.673  & 1.739  & 1.676  & 1.696  & 1.414  & 5.74 \\
    TATO               & Data-side adaptation    & 1.742   & 1.621   & 1.704   & 1.689   & 1.375   & 7.19 \\
    \midrule
    \rowcolor{bestgray}\introact{} & Frozen governance & \best{1.366} & \best{1.536} & \second{1.367} & \best{1.423} & \best{1.223} & \best{4.11} \\
    \midrule
...
almost never intervenes can report a low rate without contributing anything. \introact{} executes an intervention on 51\% of the requests, so its accuracy is not bought by staying still, while the published baselines repair or transform every incomplete request by construction, so their intervention rate is one. Conditional on intervening it is harmful on 42.4\% of those requests against 42.7\% for the fixed intervention and 43.9\% for the simple selector. Harmful loss, which weights each harmful intervention by the loss it added, is 0.0294, against the best fixed intervention 0.0536, the simple selector 0.0505, SAITS 0.1065, TATO 0.2937. The selective rule is therefore both more accurate and cheaper when wrong.
Beneficial precision is 57.6\%. Missed opportunity is 49.2\%, which is what the reference option costs.
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.423 & 51.1\% & 42.4\% & 0.0294 & 57.6\% & 49.2\% \\
    \bottomrule
  \end{tabular}}
  \vspace{-2pt}
...
Against the untouched input the frozen configuration holds at every level: 10\% 1.423 against 1.576, 30\% 1.466 against 1.794, 50\% 1.580 against 1.875. The method holds the lowest macro average at every level, and the rows below it change order between levels, so the sweep separates the method from the roster rather than reproducing one ranking. The configuration is frozen on a bank that mixes the three levels and is not retuned per level. On the held-out family the frozen procedure improves on the untouched input, 1.367 against 1.579 with a paired interval that excludes zero, its average rank is 4.28 against the 4.14 of the best fixed intervention, and the lowest macro average there belongs to the best fixed intervention at 1.353.
...
Table~\ref{tab:ablation} reports accuracy, harm, and cost together. Two parts of the design carry the result, and the state blocks matter less than either. Intervals are paired differences on the primary backbone and table columns average over the three. Scoring an action by its global mean utility instead of its neighbourhood costs -0.0195 [-0.0362, -0.0039] and drives the intervention rate to 100\%, which is what a corpus-level answer to a per-request question looks like. Dropping the intervention block of the state changes accuracy by +0.0050 [+0.0006, +0.0094], so on that backbone the block does not pay for itself. Dropping the reference-forecast block changes it by +0.0026 [-0.0056, +0.0107], and over the three backbones the two land at 1.424 and 1.445 against 1.423, so only the second block shows up in the average.
Removing the reference option and executing the best-scoring action on every request changes accuracy by +0.0086 [-0.0008, +0.0177] while raising harmful loss from 0.0294 to 0.0505, so keeping the input is what lets the method intervene on 49\% fewer requests without paying for it in accuracy. Replacing retrieval by a per-action linear utility predictor over the same features costs -0.0303 [-0.0507, -0.0118], the largest gap in the table and the clearest sign that the local estimate is doing the work. A request costs 1.51 forecasting-backbone calls on average.
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccccc}
    ...
    \midrule
    Full \introact{}          & 1.423   & 51.1\%   & 42.4\%   & 0.0294   & 1.51 \\
    A1 Global utility          & 1.462     & 100.0\%     & 43.9\%     & 0.0642     & 2.00 \\
    A2 w/o intervention state  & 1.424     & 51.8\%     & 42.4\%     & 0.0287     & 1.52 \\
    A3 w/o reference-forecast state & 1.445 & 54.3\%     & 43.4\%     & 0.0332     & 1.54 \\
    A4 Always act              & 1.434     & 100.0\%     & 43.1\%     & 0.0505     & 2.00 \\
...
The evaluation supports three claims. Deciding per request lowers source-macro MASE from 1.576 to 1.423, the paired difference against the best fixed intervention excludes zero on Bolt, and the average rank over the deployable rows is 4.11 against 4.37 for the best fixed intervention. The reference action is returned on 49\% of requests, which lowers harmful loss relative to executing an action every time without costing accuracy. The same procedure, with its two hyperparameters chosen by the same cross-validation rule on the backbone's own replay bank, still improves on the untouched input on a backbone family that took no part in method design. One fixed repair reaches both a lower average and a lower rank there, and the interval of that difference covers zero, so the two are not separated on accuracy.
...
held-out backbone the macro average falls with every step up in history, from 1.406
to 1.366 and from 1.474 to 1.367. On TimesFM the three levels
sit within 0.007 of each other, so history neither helps nor hurts there. The
...
\begin{table}[h]
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    $25\%$  & 1.469  & 61.0\%  & 40.7\%  & 0.0332  & 1.61 \\
    $50\%$  & 1.438  & 54.0\%  & 42.1\%  & 0.0325  & 1.54 \\
    $100\%$ & 1.423 & 51.1\% & 42.4\% & 0.0294 & 1.51 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{llcccc}
    ...
    \midrule
    Bolt & the untouched input & -0.1002 & [-0.1530, -0.0496] & 0.0005 & 0.0025 \\
    Bolt & the best fixed intervention & -0.0195 & [-0.0362, -0.0039] & 0.0110 & 0.0220 \\
    Bolt & the simple selector & -0.0089 & [-0.0316, +0.0139] & 0.5230 & 0.5230 \\
    Bolt & SAITS & -0.3067 & [-0.3928, -0.2231] & 0.0005 & 0.0025 \\
    Bolt & TATO & -0.3763 & [-0.5606, -0.2223] & 0.0005 & 0.0025 \\
    TimesFM & the untouched input & -0.1464 & [-0.1985, -0.0932] & 0.0005 & 0.0025 \\
    TimesFM & the best fixed intervention & -0.0404 & [-0.0983, +0.0178] & 0.3490 & 1.0000 \\
    TimesFM & the simple selector & -0.0227 & [-0.0628, +0.0175] & 0.4760 & 1.0000 \\
    TimesFM & SAITS & -0.2031 & [-0.4509, +0.0425] & 0.4080 & 1.0000 \\
    TimesFM & TATO & -0.0853 & [-0.3052, +0.1273] & 0.5120 & 1.0000 \\
    Chronos-2 & the untouched input & -0.2119 & [-0.3053, -0.1203] & 0.0005 & 0.0025 \\
    Chronos-2 & the best fixed intervention & +0.0149 & [-0.0150, +0.0451] & 0.3550 & 0.7100 \\
    Chronos-2 & the simple selector & -0.0674 & [-0.1706, +0.0352] & 0.4820 & 0.7100 \\
    Chronos-2 & SAITS & -0.3090 & [-0.4936, -0.1251] & 0.0005 & 0.0025 \\
    Chronos-2 & TATO & -0.3361 & [-0.4989, -0.1817] & 0.0005 & 0.0025 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 0.879 & 1.290 & 1.324 & 1.007 & 0.911 & 2.397 & 0.912 & 0.614 & 1.167 \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.031 & 1.346 & 1.383 & 1.220 & 0.904 & 4.463 & 1.030 & 1.148 & 1.566 \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.024 & 1.223 & 1.165 & 1.014 & 0.915 & 3.195 & 0.901 & 0.668 & 1.263 \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.119 & 1.282 & 1.305 & 1.213 & 0.917 & 6.459 & 1.009 & 1.161 & 1.808 \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.038 & 1.199 & 1.232 & 1.008 & 0.792 & 2.031 & 0.857 & 0.665 & 1.103 \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.149 & 1.320 & 1.315 & 1.237 & 0.872 & 4.942 & 0.967 & 1.256 & 1.632 \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.383 & 1.187 & 1.620 & 1.423 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    \midrule
    $\times 0.5$ & 1.383  & 1.064  & 1.496  & 1.423 \\
    $\times 1.0$ & 1.383   & 1.187   & 1.620   & 1.423 \\
    $\times 2.0$ & 1.383   & 1.458   & 2.141   & 1.423 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccccc}
    ...
    \midrule
    \textsc{Native Keep} & 1.436 & 1.379 & 1.400 & 2.088 & 5.49 \\
    \textsc{Best Fixed}  & 1.416   & 1.391   & 1.401   & 1.545   & 4.37 \\
    R2-CART              & 1.409   & 1.357   & 1.489   & 1.568   & 4.85 \\
    SAITS & 1.272 & 1.526 & 1.616 & 2.370 & 5.74 \\
    TATO & 1.432 & 1.429 & 1.468 & 2.427 & 7.19 \\
    \midrule
    \rowcolor{bestgray}\introact{} & 1.380 & 1.378 & 1.404 & 1.531 & 4.11 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    ...
    \midrule
    \textsc{Native Keep} & 1.576 & 1.325 & 5.49 \\
    \textsc{Best Fixed} & 1.438 & 1.233 & 4.37 \\
    R2-CART & 1.456 & 1.244 & 4.85 \\
    SAITS & 1.696 & 1.414 & 5.74 \\
    TATO & 1.689 & 1.375 & 7.19 \\
    \midrule
    \rowcolor{bestgray}\introact{} & 1.423 & 1.223 & 4.11 \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    ...
    Method & MASE $\downarrow$ & RMSSE $\downarrow$ & Rank $\downarrow$ \\
    \midrule
    \textsc{Native Keep} & 1.794 & 1.439 & 5.69 \\
    \textsc{Best Fixed} & 1.483 & 1.252 & 4.48 \\
    R2-CART & 1.469 & 1.251 & 4.55 \\
    SAITS & 1.798 & 1.472 & 6.01 \\
    TATO & 1.939 & 1.517 & 7.29 \\
    \midrule
    \rowcolor{bestgray}\introact{} & 1.466 & 1.252 & 3.80 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    ...
    Method & MASE $\downarrow$ & RMSSE $\downarrow$ & Rank $\downarrow$ \\
    \midrule
    \textsc{Native Keep} & 1.875 & 1.486 & 5.92 \\
    \textsc{Best Fixed} & 1.605 & 1.318 & 4.32 \\
    R2-CART & 1.763 & 1.423 & 4.56 \\
    SAITS & 1.804 & 1.474 & 5.97 \\
    TATO & 2.023 & 1.551 & 7.44 \\
    \midrule
    \rowcolor{bestgray}\introact{} & 1.580 & 1.319 & 3.78 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    \midrule
    \rowcolor{bestgray}\introact{} & 1.423 & 1.466 & 1.580 & +0.087 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
Over 8550 admissible pairs the sign of the score and the sign of the realised utility agree on 53\% of them, and the mean realised utility is +0.173 where the score is positive against -0.028 where it is negative, so the score carries the direction the decision needs without carrying the level.
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    \midrule
    Full \introact{}            & 1.423 & 1.223 & 51.1\% & 42.4\% & 0.0294 & 1.51 \\
    A1 Global utility & 1.462 & 1.246 & 100.0\% & 43.9\% & 0.0642 & 2.00 \\
    A2 w/o intervention state & 1.424 & 1.224 & 51.8\% & 42.4\% & 0.0287 & 1.52 \\
    A3 w/o reference-forecast state & 1.445 & 1.239 & 54.3\% & 43.4\% & 0.0332 & 1.54 \\
    A4 Always act & 1.434 & 1.226 & 100.0\% & 43.1\% & 0.0505 & 2.00 \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccccc}
    ...
    Bolt & 50\% & 8 & 1.64 & 1.393 & 26.8\% \\
    Bolt & 50\% & 32 & 1 & 1.364 & 50.3\% \\
    Bolt & 50\% & 8 & 1 & 1.384 & 48.2\% \\
    Bolt & 75\% & 128 & 1 & 1.366 & 53.0\% \\
    Bolt & 75\% & 32 & 0.5 & 1.361 & 61.1\% \\
    Bolt & 75\% & 16 & 1.64 & 1.404 & 25.1\% \\
    TimesFM & full bank & 16 & 1 & 1.536 & 69.2\% \\
    TimesFM & 50\% & 8 & 0 & 1.528 & 89.3\% \\
    TimesFM & 50\% & 32 & 1 & 1.536 & 78.8\% \\
    TimesFM & 50\% & 32 & 0 & 1.544 & 96.1\% \\
    TimesFM & 75\% & 16 & 0 & 1.538 & 91.6\% \\
    TimesFM & 75\% & 16 & 0.5 & 1.532 & 82.4\% \\
    TimesFM & 75\% & 32 & 1 & 1.536 & 78.8\% \\
    Chronos-2 & full bank & 16 & 1.64 & 1.367 & 31.1\% \\
    Chronos-2 & 50\% & 8 & 1.64 & 1.410 & 29.7\% \\
    Chronos-2 & 50\% & 16 & 1.64 & 1.367 & 31.1\% \\
    Chronos-2 & 50\% & 8 & 1.64 & 1.410 & 29.7\% \\
    Chronos-2 & 75\% & 16 & 1.64 & 1.367 & 31.1\% \\
    Chronos-2 & 75\% & 8 & 1.64 & 1.410 & 29.7\% \\
    Chronos-2 & 75\% & 8 & 1.64 & 1.410 & 29.7\% \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\textsc{Keep} on each realisation. The simple selector moves by 0.005 on
Bolt and 0.039 on the held-out backbone over the same three realisations,
...
\resizebox{\textwidth}{!}{\begin{tabular}{llccc}
    ...
    \midrule
    Bolt      & primary seed        & 1.366 & -0.100 & 53.0\% \\
    Bolt      & second realisation  & 1.367 & -0.094 & 52.5\% \\
    Bolt      & third realisation   & 1.359 & -0.104 & 53.0\% \\
    \midrule
    Chronos-2 & primary seed        & 1.367 & -0.212 & 31.1\% \\
    Chronos-2 & second realisation  & 1.396 & -0.180 & 32.0\% \\
    Chronos-2 & third realisation   & 1.372 & -0.230 & 33.4\% \\
    \midrule
    \multicolumn{2}{l}{Spread over the three realisations (Bolt)}      & 0.008 & --- & 0.5\% \\
    \multicolumn{2}{l}{Spread over the three realisations (Chronos-2)} & 0.029  & --- & 2.4\% \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    \rowcolor{bestgray}\introact{} & 8,471 calls, once per backbone & 4 per request & 1.51 & 80 ms & 116 ms & 939 ms \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    ...
    \midrule
    \textsc{Keep} returned (input untouched)   & 372    & 48.9\%    & 1 \\
    Intervention executed (input changed)      & 388     & 51.1\%     & 2 \\
