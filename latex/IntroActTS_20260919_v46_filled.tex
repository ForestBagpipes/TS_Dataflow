...
Against the untouched input the paired difference is Bolt -0.1002 [-0.1530, -0.0496], TimesFM -0.1464 [-0.1985, -0.0932], Chronos-2 -0.2119 [-0.3053, -0.1203], and every interval excludes zero. Against the best fixed intervention the interval excludes zero on Bolt. It is the lowest deployable row on Bolt and TimesFM, and on Chronos-2 one fixed repair reaches a lower macro average, which \S\ref{sec:exp-robust} returns to. The deployable rows span 1.423 to 1.696 and the catalog oracle sits at 1.307, so the room to recover is bounded and the gaps between rows are a visible part of it. Both published baselines repair or transform every incomplete request, and SAITS ends at 1.696 and TATO ends at 1.689, above the 1.576 of leaving the input alone. SAITS is below it on 18 of 24 source and backbone cells, so what costs that repair its average is the minority of cells on which it is badly wrong, and that is the failure a per-request rule can avoid. TATO is below it on only 5 of 24 such cells, so that one loses broadly rather than on a few cells. Against the simple selector no interval excludes zero, so accuracy alone does not separate the two and what does is in Table~\ref{tab:harm}.
Average rank over the evaluation cells is Bolt 4.05, TimesFM 3.99, Chronos-2 4.28, the best of the six deployable rows on Bolt and TimesFM. On Chronos-2 the best fixed intervention ranks 4.14 against our 4.28. Averaged over the three backbones the rank is 4.11 against 4.37 for the best fixed intervention, which is the closest deployable row. Per-source MASE, MAE, and RMSE are in Appendix~\ref{app:datasets}, and
...
Table~\ref{tab:ablation} reports accuracy, harm, and cost together. Two parts of the design carry the result, and the state blocks matter less than either. Each interval below is the paired difference on the primary backbone, while the table columns average over the three. Scoring an action by its global mean utility instead of its neighbourhood costs -0.0195 [-0.0362, -0.0039] and drives the intervention rate to 100\%, which is what a corpus-level answer to a per-request question looks like. Dropping the intervention block of the state changes accuracy by +0.0050 [+0.0006, +0.0094], so on that backbone the block does not pay for itself. Dropping the reference-forecast block changes it by +0.0026 [-0.0056, +0.0107]. Averaged over the three backbones the first ablation lands at 1.424 and the second at 1.445 against 1.423 for the full method, so the reference-forecast block is the one that shows up in the average.
