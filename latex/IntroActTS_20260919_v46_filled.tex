...
The evaluation supports three claims. Deciding per request lowers source-macro MASE from 1.576 to 1.447, and the lowest macro average there belongs to the best fixed intervention at 1.353. The reference action is returned on 17\% of requests, which lowers harmful loss relative to executing an action every time without costing accuracy. The same procedure, with its two hyperparameters chosen by the same cross-validation rule on the backbone's own replay bank, holds on a backbone family that took no part in method design.
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    $\times 1.0$ & 0.000000   & 0.109   & 0.441   & \ph{OPPSENS_ONE_OVR} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    SAITS              & 1.696      & 1.798      & 1.804      & \ph{RB_SAITS_WORST} \\
    TATO               & 1.689    & 1.939    & 2.023    & \ph{RB_TATO_WORST} \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    \midrule
    SAITS                 & 8 imputers, 3 min total     & 1 per request     & 1.00     & \ph{SAITS_MEAN}     & \ph{SAITS_P95}     & \ph{SAITS_MAX} \\
    TATO               & 3,072 calls, once per backbone   & 1 per request   & 1.00   & \ph{TATO_MEAN}   & \ph{TATO_P95}   & \ph{TATO_MAX} \\
    \rowcolor{bestgray}\introact{} & 8,471 calls, once per backbone & 4 per request & 1.83 & \ph{OURS_MEAN} & \ph{OURS_P95} & \ph{OURS_MAX} \\
    \midrule
