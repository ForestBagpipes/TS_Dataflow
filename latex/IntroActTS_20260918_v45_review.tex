...
The bank is indexed by the frozen backbone revision and by the source, and it covers only the
missingness protocol registered in advance. Both properties are load-bearing, and
Appendix~\ref{app:replay} states what they imply for the scope of the method.
...
on the gate stays below a pre-registered cap, we take the point with the best gate MASE, so
harm enters as a constraint rather than as a quantity to trade against accuracy. No evaluation
data is used.
...
ChannelTokenFormer~\citep{jang2026channeltokenformer}. The four data-side methods compose with
each frozen backbone, whereas ChannelTokenFormer is a complete forecasting architecture trained
for the task and therefore has no per-frozen-backbone column. Three controls accompany them:
\textsc{Native Keep}, \textsc{Best Fixed}, and \textbf{R2-CART}. A \emph{Catalog Oracle}
that reads the future target is reported as a diagnostic reference and excluded from all ranks.
The severity matrix of \S\ref{sec:exp-robust} uses a reduced but type-complete set in which
T1~\citep{park2026t1} stands for reconstruction-based imputation and
TATO~\citep{qiu2026tato} for data-side adaptation, and Appendix~\ref{app:datasets} documents
the baseline roster, the held-back VIDA comparison, and the per-source tables.
...
Appendix~\ref{app:metric-conventions} gives the full metric and statistics conventions.
...
summary. Figure~\ref{fig:why-selective}(a) shows the comparison, and
Appendix~\ref{app:recutils} adds the Reconstruction Oracle diagnostic.
...
\begin{table}[t]
  ...
  and lower is better everywhere. \emph{Type} separates data-side methods, which compose with a
  frozen backbone, from ChannelTokenFormer, a complete forecasting architecture trained for the
  task that has no per-frozen-backbone column. \best{Bold} marks the best deployable method and
  \second{underline} the second best. The oracle row reads the future target for diagnosis
...
\resizebox{\textwidth}{!}{\begin{tabular}{llccccc c}
    ...
    TOI-VSF            & VSF                     & \ph{TOIVSF_BOLT} & \ph{TOIVSF_TF} & \ph{TOIVSF_CH2} & \ph{TOIVSF_OVR} & \ph{TOIVSF_RMSSE} & \ph{TOIVSF_RANK} \\
    GIMCC              & VSF                     & \ph{GIMCC_BOLT}  & \ph{GIMCC_TF}  & \ph{GIMCC_CH2}  & \ph{GIMCC_OVR}  & \ph{GIMCC_RMSSE}  & \ph{GIMCC_RANK} \\
    SRDI               & VSF                     & \ph{SRDI_BOLT}   & \ph{SRDI_TF}   & \ph{SRDI_CH2}   & \ph{SRDI_OVR}   & \ph{SRDI_RMSSE}   & \ph{SRDI_RANK} \\
    ChannelTokenFormer & Forecast model          & N/A\textsuperscript{*} & N/A\textsuperscript{*} & N/A\textsuperscript{*} & \ph{CTF_OVR} & \ph{CTF_RMSSE} & \ph{CTF_RANK} \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    TOI-VSF            & \ph{HARM_MASE_TOIVSF} & \ph{HARM_IR_TOIVSF} & \ph{HARM_HIR_TOIVSF} & \ph{HARM_HL_TOIVSF} & \ph{HARM_BP_TOIVSF} & \ph{HARM_MO_TOIVSF} \\
    GIMCC              & \ph{HARM_MASE_GIMCC}  & \ph{HARM_IR_GIMCC}  & \ph{HARM_HIR_GIMCC}  & \ph{HARM_HL_GIMCC}  & \ph{HARM_BP_GIMCC}  & \ph{HARM_MO_GIMCC} \\
    SRDI               & \ph{HARM_MASE_SRDI}   & \ph{HARM_IR_SRDI}   & \ph{HARM_HIR_SRDI}   & \ph{HARM_HL_SRDI}   & \ph{HARM_BP_SRDI}   & \ph{HARM_MO_SRDI} \\
    ChannelTokenFormer & \ph{HARM_MASE_CTF}    & \ph{HARM_IR_CTF}    & \ph{HARM_HIR_CTF}    & \ph{HARM_HL_CTF}    & \ph{HARM_BP_CTF}    & \ph{HARM_MO_CTF} \\
    \midrule
  ...
  \end{tabular}}
  \vspace{-2pt}
  {\footnotesize Methods that always return a repaired input intervene on every incomplete
  request by construction, so their intervention rate is one and their harmful rate is
  unconditional.}
\end{table}
...
unseen-severity stress test. Table~\ref{tab:robust} reports the severity trend, the worst
pattern cell, and the held-out Chronos-2 backbone under an unchanged configuration.
\ph{ROBUST_READING}. \ph{ROBUST_30}. \ph{ROBUST_50}. \ph{ROBUST_WORST}. \ph{ROBUST_CH2}.
Appendix~\ref{app:severity-full} holds the complete per-severity and per-pattern tables.

\begin{table}[t]
  \caption{Within-grid robustness to missingness severity, with no method retuned between
  levels. The worst-pattern column is the largest degradation relative to \textsc{Native Keep}
  over all pattern and backbone cells at that severity. The Chronos-2 column is the held-out
  backbone under an unchanged configuration.}
  \label{tab:robust}
  \centering
  \small
...
main-text tables. Section~E carries the ablations, the replay-size study, and the sensitivity
grid. Sections~F and~G record cost and reproducibility. A final unnumbered section holds
exploratory material that does not support any claim in the main text.
...
\resizebox{\textwidth}{!}{\begin{tabular}{lll}
    ...
    \midrule
    A. Protocol and method details & final TRAIN and TEST split with purging, scope and assumptions, the action catalog, the state feature definitions, the registered missingness patterns, the complete-input contract & what exactly was fixed before the evaluation ran, and which claims are out of scope? \\
    ...
    E. Ablations, replay size, and sensitivity & the full ablation vector, every method re-fitted at $25/50/100\%$ support, the $k$ and $\beta$ gate grid, mask-realisation stability & which component carries the gain, how much history is needed, and is the result an artefact of one mask? \\
    ...
    Supplementary (unnumbered) & the preceding development cycle and the finance case study & what was tried and rejected, and what lies outside the benchmark sources? \\
    \bottomrule
  \end{tabular}}
\end{table}
...
describe another, and because the state distribution differs across sources, the bank is built
per frozen backbone revision and per source. Pseudo-deployment severities are drawn from
...
MASE~\citep{hyndman2006mase} is the primary metric and RMSSE is the secondary metric. For both,
the denominator is the mean absolute first difference of the target channel computed on the
TRAIN portion of that source, so the scaling constant of a source never depends on the
evaluation block. The denominator is computed per channel before any aggregation over
channels, and the per-source seasonal periods used by the seasonal variants are listed in
Table~\ref{tab:app-seasonality}. Aggregation is source-macro, first over parents within a
source and then equally over the eight sources, and the same order is applied inside every
bootstrap resample. MAE and RMSE are reported per source and are never averaged across
sources, because the sources have different scales and a cross-source average of a raw error
has no common unit.

\begin{table}[h]
  \caption{Per-source seasonal periods and context conventions used by the metric
  denominators. Context length is $L = 512$ for every source.}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    \toprule
    Source & Seasonal period & Channels & Scaling constant source \\
    \midrule
    ETTh1 / ETTh2 & 24 & 7 & TRAIN first difference of the target channel \\
    ETTm1 / ETTm2 & 96 & 7 & TRAIN first difference of the target channel \\
    Electricity & 24 & 321 & TRAIN first difference of the target channel \\
    Exchange & 7 & 8 & TRAIN first difference of the target channel \\
    Traffic & 24 & 862 & TRAIN first difference of the target channel \\
    Weather & 144 & 21 & TRAIN first difference of the target channel \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    TOI-VSF & \ph{SRC_BOLT_TOIVSF_96_ETTH1} & \ph{SRC_BOLT_TOIVSF_96_ETTH2} & \ph{SRC_BOLT_TOIVSF_96_ETTM1} & \ph{SRC_BOLT_TOIVSF_96_ETTM2} & \ph{SRC_BOLT_TOIVSF_96_ELEC} & \ph{SRC_BOLT_TOIVSF_96_EXCH} & \ph{SRC_BOLT_TOIVSF_96_TRAF} & \ph{SRC_BOLT_TOIVSF_96_WEA} & \ph{SRC_BOLT_TOIVSF_96_MACRO} \\
    GIMCC & \ph{SRC_BOLT_GIMCC_96_ETTH1} & \ph{SRC_BOLT_GIMCC_96_ETTH2} & \ph{SRC_BOLT_GIMCC_96_ETTM1} & \ph{SRC_BOLT_GIMCC_96_ETTM2} & \ph{SRC_BOLT_GIMCC_96_ELEC} & \ph{SRC_BOLT_GIMCC_96_EXCH} & \ph{SRC_BOLT_GIMCC_96_TRAF} & \ph{SRC_BOLT_GIMCC_96_WEA} & \ph{SRC_BOLT_GIMCC_96_MACRO} \\
    ...
    ChannelTokenFormer & \ph{SRC_BOLT_CTF_96_ETTH1} & \ph{SRC_BOLT_CTF_96_ETTH2} & \ph{SRC_BOLT_CTF_96_ETTM1} & \ph{SRC_BOLT_CTF_96_ETTM2} & \ph{SRC_BOLT_CTF_96_ELEC} & \ph{SRC_BOLT_CTF_96_EXCH} & \ph{SRC_BOLT_CTF_96_TRAF} & \ph{SRC_BOLT_CTF_96_WEA} & \ph{SRC_BOLT_CTF_96_MACRO} \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    TOI-VSF & \ph{SRC_BOLT_TOIVSF_192_ETTH1} & \ph{SRC_BOLT_TOIVSF_192_ETTH2} & \ph{SRC_BOLT_TOIVSF_192_ETTM1} & \ph{SRC_BOLT_TOIVSF_192_ETTM2} & \ph{SRC_BOLT_TOIVSF_192_ELEC} & \ph{SRC_BOLT_TOIVSF_192_EXCH} & \ph{SRC_BOLT_TOIVSF_192_TRAF} & \ph{SRC_BOLT_TOIVSF_192_WEA} & \ph{SRC_BOLT_TOIVSF_192_MACRO} \\
    GIMCC & \ph{SRC_BOLT_GIMCC_192_ETTH1} & \ph{SRC_BOLT_GIMCC_192_ETTH2} & \ph{SRC_BOLT_GIMCC_192_ETTM1} & \ph{SRC_BOLT_GIMCC_192_ETTM2} & \ph{SRC_BOLT_GIMCC_192_ELEC} & \ph{SRC_BOLT_GIMCC_192_EXCH} & \ph{SRC_BOLT_GIMCC_192_TRAF} & \ph{SRC_BOLT_GIMCC_192_WEA} & \ph{SRC_BOLT_GIMCC_192_MACRO} \\
    ...
    ChannelTokenFormer & \ph{SRC_BOLT_CTF_192_ETTH1} & \ph{SRC_BOLT_CTF_192_ETTH2} & \ph{SRC_BOLT_CTF_192_ETTM1} & \ph{SRC_BOLT_CTF_192_ETTM2} & \ph{SRC_BOLT_CTF_192_ELEC} & \ph{SRC_BOLT_CTF_192_EXCH} & \ph{SRC_BOLT_CTF_192_TRAF} & \ph{SRC_BOLT_CTF_192_WEA} & \ph{SRC_BOLT_CTF_192_MACRO} \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    TOI-VSF & \ph{SRC_TF_TOIVSF_96_ETTH1} & \ph{SRC_TF_TOIVSF_96_ETTH2} & \ph{SRC_TF_TOIVSF_96_ETTM1} & \ph{SRC_TF_TOIVSF_96_ETTM2} & \ph{SRC_TF_TOIVSF_96_ELEC} & \ph{SRC_TF_TOIVSF_96_EXCH} & \ph{SRC_TF_TOIVSF_96_TRAF} & \ph{SRC_TF_TOIVSF_96_WEA} & \ph{SRC_TF_TOIVSF_96_MACRO} \\
    GIMCC & \ph{SRC_TF_GIMCC_96_ETTH1} & \ph{SRC_TF_GIMCC_96_ETTH2} & \ph{SRC_TF_GIMCC_96_ETTM1} & \ph{SRC_TF_GIMCC_96_ETTM2} & \ph{SRC_TF_GIMCC_96_ELEC} & \ph{SRC_TF_GIMCC_96_EXCH} & \ph{SRC_TF_GIMCC_96_TRAF} & \ph{SRC_TF_GIMCC_96_WEA} & \ph{SRC_TF_GIMCC_96_MACRO} \\
    ...
    ChannelTokenFormer & \ph{SRC_TF_CTF_96_ETTH1} & \ph{SRC_TF_CTF_96_ETTH2} & \ph{SRC_TF_CTF_96_ETTM1} & \ph{SRC_TF_CTF_96_ETTM2} & \ph{SRC_TF_CTF_96_ELEC} & \ph{SRC_TF_CTF_96_EXCH} & \ph{SRC_TF_CTF_96_TRAF} & \ph{SRC_TF_CTF_96_WEA} & \ph{SRC_TF_CTF_96_MACRO} \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    TOI-VSF & \ph{SRC_TF_TOIVSF_192_ETTH1} & \ph{SRC_TF_TOIVSF_192_ETTH2} & \ph{SRC_TF_TOIVSF_192_ETTM1} & \ph{SRC_TF_TOIVSF_192_ETTM2} & \ph{SRC_TF_TOIVSF_192_ELEC} & \ph{SRC_TF_TOIVSF_192_EXCH} & \ph{SRC_TF_TOIVSF_192_TRAF} & \ph{SRC_TF_TOIVSF_192_WEA} & \ph{SRC_TF_TOIVSF_192_MACRO} \\
    GIMCC & \ph{SRC_TF_GIMCC_192_ETTH1} & \ph{SRC_TF_GIMCC_192_ETTH2} & \ph{SRC_TF_GIMCC_192_ETTM1} & \ph{SRC_TF_GIMCC_192_ETTM2} & \ph{SRC_TF_GIMCC_192_ELEC} & \ph{SRC_TF_GIMCC_192_EXCH} & \ph{SRC_TF_GIMCC_192_TRAF} & \ph{SRC_TF_GIMCC_192_WEA} & \ph{SRC_TF_GIMCC_192_MACRO} \\
    ...
    ChannelTokenFormer & \ph{SRC_TF_CTF_192_ETTH1} & \ph{SRC_TF_CTF_192_ETTH2} & \ph{SRC_TF_CTF_192_ETTM1} & \ph{SRC_TF_CTF_192_ETTM2} & \ph{SRC_TF_CTF_192_ELEC} & \ph{SRC_TF_CTF_192_EXCH} & \ph{SRC_TF_CTF_192_TRAF} & \ph{SRC_TF_CTF_192_WEA} & \ph{SRC_TF_CTF_192_MACRO} \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    TOI-VSF & \ph{SRC_CH2_TOIVSF_96_ETTH1} & \ph{SRC_CH2_TOIVSF_96_ETTH2} & \ph{SRC_CH2_TOIVSF_96_ETTM1} & \ph{SRC_CH2_TOIVSF_96_ETTM2} & \ph{SRC_CH2_TOIVSF_96_ELEC} & \ph{SRC_CH2_TOIVSF_96_EXCH} & \ph{SRC_CH2_TOIVSF_96_TRAF} & \ph{SRC_CH2_TOIVSF_96_WEA} & \ph{SRC_CH2_TOIVSF_96_MACRO} \\
    GIMCC & \ph{SRC_CH2_GIMCC_96_ETTH1} & \ph{SRC_CH2_GIMCC_96_ETTH2} & \ph{SRC_CH2_GIMCC_96_ETTM1} & \ph{SRC_CH2_GIMCC_96_ETTM2} & \ph{SRC_CH2_GIMCC_96_ELEC} & \ph{SRC_CH2_GIMCC_96_EXCH} & \ph{SRC_CH2_GIMCC_96_TRAF} & \ph{SRC_CH2_GIMCC_96_WEA} & \ph{SRC_CH2_GIMCC_96_MACRO} \\
    ...
    ChannelTokenFormer & \ph{SRC_CH2_CTF_96_ETTH1} & \ph{SRC_CH2_CTF_96_ETTH2} & \ph{SRC_CH2_CTF_96_ETTM1} & \ph{SRC_CH2_CTF_96_ETTM2} & \ph{SRC_CH2_CTF_96_ELEC} & \ph{SRC_CH2_CTF_96_EXCH} & \ph{SRC_CH2_CTF_96_TRAF} & \ph{SRC_CH2_CTF_96_WEA} & \ph{SRC_CH2_CTF_96_MACRO} \\
    \midrule
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    TOI-VSF & \ph{SRC_CH2_TOIVSF_192_ETTH1} & \ph{SRC_CH2_TOIVSF_192_ETTH2} & \ph{SRC_CH2_TOIVSF_192_ETTM1} & \ph{SRC_CH2_TOIVSF_192_ETTM2} & \ph{SRC_CH2_TOIVSF_192_ELEC} & \ph{SRC_CH2_TOIVSF_192_EXCH} & \ph{SRC_CH2_TOIVSF_192_TRAF} & \ph{SRC_CH2_TOIVSF_192_WEA} & \ph{SRC_CH2_TOIVSF_192_MACRO} \\
    GIMCC & \ph{SRC_CH2_GIMCC_192_ETTH1} & \ph{SRC_CH2_GIMCC_192_ETTH2} & \ph{SRC_CH2_GIMCC_192_ETTM1} & \ph{SRC_CH2_GIMCC_192_ETTM2} & \ph{SRC_CH2_GIMCC_192_ELEC} & \ph{SRC_CH2_GIMCC_192_EXCH} & \ph{SRC_CH2_GIMCC_192_TRAF} & \ph{SRC_CH2_GIMCC_192_WEA} & \ph{SRC_CH2_GIMCC_192_MACRO} \\
    ...
    ChannelTokenFormer & \ph{SRC_CH2_CTF_192_ETTH1} & \ph{SRC_CH2_CTF_192_ETTH2} & \ph{SRC_CH2_CTF_192_ETTM1} & \ph{SRC_CH2_CTF_192_ETTM2} & \ph{SRC_CH2_CTF_192_ELEC} & \ph{SRC_CH2_CTF_192_EXCH} & \ph{SRC_CH2_CTF_192_TRAF} & \ph{SRC_CH2_CTF_192_WEA} & \ph{SRC_CH2_CTF_192_MACRO} \\
    \midrule
...
If a core baseline turns out not to admit an admissible adaptation to our protocol, we report that
...
type-complete set. No method is retuned between severities. \introact{} reuses the $k$ and
$\beta$ frozen on the gate, and every baseline reuses the models and rules fixed on TRAIN.
The $10\%$ column is the main-text condition. The $30\%$ and $50\%$ columns test whether one
frozen configuration holds across the registered severity grid, and they are not a
generalisation test to unseen severity, because the replay bank already samples from the same
three levels.
...
\begin{table}[h]
  ...
  $10\%$ table and nothing is retuned.}
  \label{tab:app-sev30}
  \centering
  \small
  \setlength{\tabcolsep}{4pt}
...
\begin{table}[h]
  \caption{Complete results at $50\%$ severity. This level is inside the registered grid that
  the replay bank samples from, so the row shows within-grid robustness of one frozen
  configuration and not behaviour on unseen severity.}
  \label{tab:app-sev50}
  \centering
  \small
  \setlength{\tabcolsep}{4pt}
...
methods that produce an explicit imputation of the hidden positions, so \textsc{Keep}, the
transformation-based actions, and a selected intervention are not placed on a common
reconstruction scale. The comparison is therefore restricted to the methods that emit
reconstructed values, and the ranking analysis is performed within each parent rather than on
a pooled sample, because parents differ in scale and in difficulty. Table~\ref{tab:app-recutils}
...
\section{Ablations, Replay Size, and Sensitivity}
\label{app:ablations-full}
\subsection{Historical Support Results}
\label{app:support}
\label{app:support-details}
Every method is re-fitted from scratch under
...
\begin{table}[h]
  \caption{Offline and online cost by support level. Offline cost is the one-off fitting cost, and
...
Table~\ref{tab:ablation}, and Table~\ref{tab:app-ablation-stab} repeats the ablation under a
different deterministic ordering of the replay records, so that a reader can see whether a
variant's position depends on the order in which records were consumed. The variant
definitions follow \S\ref{sec:exp-ablation} exactly.
...
\begin{table}[h]
  ...
  between the two rankings of the six selector variants, per backbone. A high value means the
  variant ordering is not an artefact of record order. The Reconstruction Oracle is excluded
  because it is not a selector variant.}
  ...
  \begin{tabular}{lcc}
    \toprule
    Backbone & Spearman $\rho$ between orderings $\uparrow$ & Rank of the full variant \\
    \midrule
...
and per source, whereas the online columns are per request. Within each column a lower value
is better, and the two columns are read separately rather than combined into a single score.
Latency percentiles are measured per request and exclude cold start, which is reported on its
own line. Candidate materialisation is a separate line because it is paid before the decision
is made and is not a forecasting-backbone call.

\begin{table}[t]
  \caption{Deployment cost for all methods, offline and online kept separate. Latency
  percentiles are per request and exclude cold start, reported on its own line. Backbone calls
  counts forecasting-backbone calls per request and excludes candidate materialisation, which
  is reported in its own column.}
...
\label{app:finance}
The two finance parents available to us are reported as a case study only and are not part of
any claim of generalisation. QLIKE appears here and nowhere else. Results are reported
separately for the complete-observed condition, which is the no-op check, and for controlled
deletion, which carries the only quantitative comparison. Verified natural missingness is
reported only where it exists, and where it does not exist that is stated rather than
substituted with synthetic gaps. Finance series contain real jumps such as market closures,
and those are neither smoothed nor treated as missing observations.

\begin{table}[h]
  \caption{Finance case study. The complete-observed rows are the no-op check, and the
  controlled-deletion rows carry the only quantitative comparison. Natural missingness is
  listed only where it has been verified. No claim of financial generalisation is made from
  this table.}
