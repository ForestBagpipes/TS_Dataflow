...
% ---------------------------------------------------------------------------
%  版本      : v4.7 修订工作稿（2026-09-19，等待真实实验回填）
%  本版依据 v4.6 已查看的 TEST 结果修订，属于 post-hoc 开发。
%  以下旧版维护记录为来源历史，不代表 v4.7 已完成。
...
\begin{abstract}
When a deployed time-series foundation model is frozen, input processing remains a way to
handle incomplete context without updating its parameters. Reconstruction accuracy alone
does not determine whether an imputation improves the downstream forecast. We study which
admissible input intervention should be applied to an individual request, including whether
the input should remain unchanged. We
...
patterns and two horizons, on three frozen backbone families in a post-hoc evaluation,
\ph{ABSTRACT_RESULT}. \ph{ABSTRACT_HARM}.
\ph{ABSTRACT_CLOSING}.
\end{abstract}
...
\begin{figure}[t]
  ...
  \caption{\textbf{(a)}~Share of reused evaluation episodes on which each catalog action is oracle-best.
  The distribution describes how the oracle-best action varies across requests.
...
catalog actions, so two of the five interventions cost one call to a frozen imputation model on top
...
\begin{figure}[t]
  ...
  historical targets enter the decision layer. Evaluation separately reads targets after
  decisions are fixed. \textbf{Online}: the reference forecast is computed, an action-conditioned
...
Appendix~\ref{app:replay} compares unrestricted and restricted retrieval when those diagnostics
are available.
...
adapts the distance scale to each neighbourhood; it does not fix the effective sample size.
The constant $\epsilon$ guards the case where every retrieved distance is zero. The local mean
...
means that the estimated mean utility is offset by the chosen dispersion penalty. This is
an operational threshold, not a statistical test of equivalence to leaving the input unchanged.
...
Chronos-2~\citep{chronos2_2025}. The same selection procedure is applied to a separate replay
bank for each backbone. Chronos-2 was excluded from the initial method design, but its earlier
evaluation informed this revision. It therefore provides a cross-backbone comparison here,
not an independent held-out confirmation of the revised method.
...
For the v47 rerun, the comparison roster and acceptance gate are frozen before any new v47
evaluation is read. Three controls establish what a per-request decision has to
beat. \textsc{Native Keep} never intervenes and is the reference every other row is measured
against. \textsc{Best Fixed} applies to every request the single catalog intervention with the
lowest source-macro realised MASE on the replay bank, falling back to \textsc{Keep} when that
intervention is unsupported. This makes it the strongest policy that
...
protocol, so this row requests that trained repair on every incomplete input. It obeys the same
plausibility guard as the catalog and falls back to \textsc{Keep} when the repair is unsupported;
an execution failure remains a failure and is never relabelled as unsupported. Its bank
...
transformation pipeline is searched per source against realised futures from TRAIN. Its exact
window coverage, trial budget, failures, and adapter are reported separately so the comparison
does not imply an unverified official reproduction. A
...
block used as an internal acceptance check. Hyperparameter selection, feature normalisation,
and baseline fitting for the revised run use TRAIN only. However, this revision follows
inspection of earlier TEST results, including the Chronos-2 results. Reusing those TEST
parents constitutes post-hoc evaluation, not a new independent confirmation. The revised
configuration is frozen before its evaluation, and the manifest records that distinction.
MASE~\citep{hyndman2006mase} is the primary metric and
...
sources of different scale apart. \ph{RECON_READING}. Table~\ref{tab:rankgrid} shows the two
...
\begin{table}[t]
  ...
  over the five catalog interventions and the reused evaluation episodes of all three backbones. Each
...
\begin{tabular}{lccccc}
    ...
    \midrule
    1 (most accurate) & \ph{RG_1_1} & \ph{RG_1_2} & \ph{RG_1_3} & \ph{RG_1_4} & \ph{RG_1_5} \\
    2 & \ph{RG_2_1} & \ph{RG_2_2} & \ph{RG_2_3} & \ph{RG_2_4} & \ph{RG_2_5} \\
    3 & \ph{RG_3_1} & \ph{RG_3_2} & \ph{RG_3_3} & \ph{RG_3_4} & \ph{RG_3_5} \\
    4 & \ph{RG_4_1} & \ph{RG_4_2} & \ph{RG_4_3} & \ph{RG_4_4} & \ph{RG_4_5} \\
    5 (least accurate) & \ph{RG_5_1} & \ph{RG_5_2} & \ph{RG_5_3} & \ph{RG_5_4} & \ph{RG_5_5} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  deployable rows of this table, summarising relative ordering rather than effect size. The oracle row reads the future target for diagnosis only
  and is excluded from the ranks. The Chronos-2 configuration is
...
\resizebox{\textwidth}{!}{\begin{tabular}{llccccc c}
    ...
    Fixed SAITS        & Trained imputer         & \ph{SAITS_BOLT}  & \ph{SAITS_TF}  & \ph{SAITS_CH2}  & \ph{SAITS_OVR}  & \ph{SAITS_RMSSE}  & \ph{SAITS_RANK} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    Fixed SAITS        & \ph{HARM_MASE_SAITS}     & \ph{HARM_IR_SAITS}     & \ph{HARM_HIR_SAITS}     & \ph{HARM_HL_SAITS}     & \ph{HARM_BP_SAITS}     & \ph{HARM_MO_SAITS} \\
...
\emph{Does one frozen configuration remain useful as missingness becomes more severe, and how
does its behaviour compare across the three backbone families used in this post-hoc revision?}
...
cross-backbone comparison is the Chronos-2 column of Table~\ref{tab:main}.
...
\begin{table}[t]
  \caption{Core ablation and cost at $10\%$ severity, on the reused evaluation set. Every
...
at forecast time, so \introact{} estimates its relationship to observable state from history:
it replays TRAIN windows as pseudo-deployments,
...
arrived and is valid. \textsc{Keep}, \textsc{Ffill}, and \textsc{Context Ridge} are computed
directly from the request; two actions call the same frozen TS-ICL checkpoint; and the sixth
calls a trained imputer. The trained imputer's records in the replay bank are cross-fitted:
the parents of a source
...
and the two blocks share no origin. Hyperparameter selection, feature normalisation, baseline
fitting, and acceptance checks use TRAIN only. No TEST parent enters fitting or bank construction.
Earlier TEST outcomes nevertheless informed the revised protocol, so separation of records
does not make evaluation on the reused TEST set independent of method development.
...
\begin{table}[h]
  ...
  every statistic in the paper. TEST is the population for post-gate predictive comparison
  tables; replay-bank diagnostics, TRAIN-eval acceptance checks, and cost measurements state
  their own populations. TEST records are disjoint from both TRAIN blocks.}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    \midrule
    TEST         & \ph{TEST_PARENTS}  & \ph{TEST_ORIGINS}  & \ph{TEST_SOURCES}  & post-hoc evaluation after freezing the revised configuration \\
    \bottomrule
  \end{tabular}}
\end{table}
...
and the reported configuration is the one that reads the full bank. The quarter-bank and
full-bank MASE values are \ph{BANK_Q_BOLT} and \ph{BANK_F_BOLT} for Bolt, and
\ph{BANK_Q_CH2} and \ph{BANK_F_CH2} for Chronos-2. The spread across bank sizes for TimesFM
is \ph{BANK_SPREAD_TF}. Intervention rates are reported alongside accuracy because changing
the bank can change both neighbourhood composition and the number of requests exceeding the threshold.
...
after forecasts are seen. Replay construction enumerates all three severities
$\{10\%, 30\%, 50\%\}$ for every parent--pattern--horizon tuple. A second frozen bank-only
seed changes the deleted positions while retaining that same severity grid.
...
scale each episode by the seasonal naive error of its unmasked historical context. The same
context denominator is used for every intervention and mask variant of that window; future
targets do not enter the denominator. With $y_t$ the historical target channel and $m$
...
the registered missingness protocol, including the realised futures of those windows. No v47
fit or selection step reads a reused TEST record, although earlier TEST results informed the
v47 design as disclosed in \S\ref{sec:exp-protocol}.
...
one the evaluation produces. Five models are fitted per source: four cross-fit models and one
deployment model. The deployment model reads every
bank parent and repairs every evaluation request, and it is what the \textsc{Fixed SAITS} row
and the catalog action both call at serving time. The bank records come from four-fold
cross-fitting: all variants of one parent remain in one fold, and each fold is repaired only by
a model trained on the other three folds. Without that split the utility stored
...
in the fit contexts. For every cross-fit model, channel selection and normalisation use only
the other three folds' training contexts; the deployment model estimates both from the full
replay bank. The channel budget is fixed before any run and recorded with the configuration.
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    Fixed SAITS & \ph{SRC_BOLT_SAITS_96_ETTH1} & \ph{SRC_BOLT_SAITS_96_ETTH2} & \ph{SRC_BOLT_SAITS_96_ETTM1} & \ph{SRC_BOLT_SAITS_96_ETTM2} & \ph{SRC_BOLT_SAITS_96_ELEC} & \ph{SRC_BOLT_SAITS_96_EXCH} & \ph{SRC_BOLT_SAITS_96_TRAF} & \ph{SRC_BOLT_SAITS_96_WEA} & \ph{SRC_BOLT_SAITS_96_MACRO} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    Fixed SAITS & \ph{SRC_BOLT_SAITS_192_ETTH1} & \ph{SRC_BOLT_SAITS_192_ETTH2} & \ph{SRC_BOLT_SAITS_192_ETTM1} & \ph{SRC_BOLT_SAITS_192_ETTM2} & \ph{SRC_BOLT_SAITS_192_ELEC} & \ph{SRC_BOLT_SAITS_192_EXCH} & \ph{SRC_BOLT_SAITS_192_TRAF} & \ph{SRC_BOLT_SAITS_192_WEA} & \ph{SRC_BOLT_SAITS_192_MACRO} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    Fixed SAITS & \ph{SRC_TF_SAITS_96_ETTH1} & \ph{SRC_TF_SAITS_96_ETTH2} & \ph{SRC_TF_SAITS_96_ETTM1} & \ph{SRC_TF_SAITS_96_ETTM2} & \ph{SRC_TF_SAITS_96_ELEC} & \ph{SRC_TF_SAITS_96_EXCH} & \ph{SRC_TF_SAITS_96_TRAF} & \ph{SRC_TF_SAITS_96_WEA} & \ph{SRC_TF_SAITS_96_MACRO} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    Fixed SAITS & \ph{SRC_TF_SAITS_192_ETTH1} & \ph{SRC_TF_SAITS_192_ETTH2} & \ph{SRC_TF_SAITS_192_ETTM1} & \ph{SRC_TF_SAITS_192_ETTM2} & \ph{SRC_TF_SAITS_192_ELEC} & \ph{SRC_TF_SAITS_192_EXCH} & \ph{SRC_TF_SAITS_192_TRAF} & \ph{SRC_TF_SAITS_192_WEA} & \ph{SRC_TF_SAITS_192_MACRO} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    Fixed SAITS & \ph{SRC_CH2_SAITS_96_ETTH1} & \ph{SRC_CH2_SAITS_96_ETTH2} & \ph{SRC_CH2_SAITS_96_ETTM1} & \ph{SRC_CH2_SAITS_96_ETTM2} & \ph{SRC_CH2_SAITS_96_ELEC} & \ph{SRC_CH2_SAITS_96_EXCH} & \ph{SRC_CH2_SAITS_96_TRAF} & \ph{SRC_CH2_SAITS_96_WEA} & \ph{SRC_CH2_SAITS_96_MACRO} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccccc c}
    ...
    Fixed SAITS & \ph{SRC_CH2_SAITS_192_ETTH1} & \ph{SRC_CH2_SAITS_192_ETTH2} & \ph{SRC_CH2_SAITS_192_ETTM1} & \ph{SRC_CH2_SAITS_192_ETTM2} & \ph{SRC_CH2_SAITS_192_ELEC} & \ph{SRC_CH2_SAITS_192_EXCH} & \ph{SRC_CH2_SAITS_192_TRAF} & \ph{SRC_CH2_SAITS_192_WEA} & \ph{SRC_CH2_SAITS_192_MACRO} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    Fixed SAITS           & \ph{NOOP_SAITS}      & \ph{LOWOP_SAITS}      & \ph{HIGHOP_SAITS}      & \ph{OVR_SAITS} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccccc}
    ...
    Fixed SAITS & \ph{P1_SAITS} & \ph{P2_SAITS} & \ph{P3_SAITS} & \ph{P4_SAITS} & \ph{PR_SAITS} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    ...
    Fixed SAITS & \ph{SEV10-SAITS-MASE} & \ph{SEV10-SAITS-RMSSE} & \ph{SEV10-SAITS-RANK} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    ...
    Fixed SAITS & \ph{SEV30-SAITS-MASE} & \ph{SEV30-SAITS-RMSSE} & \ph{SEV30-SAITS-RANK} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccc}
    ...
    Fixed SAITS & \ph{SEV50-SAITS-MASE} & \ph{SEV50-SAITS-RMSSE} & \ph{SEV50-SAITS-RANK} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    Fixed SAITS        & \ph{RB_SAITS_10}      & \ph{RB_SAITS_30}      & \ph{RB_SAITS_50}      & \ph{RB_SAITS_WORST} \\
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccccccccc}
    ...
    \midrule
    \textsc{Native Keep} & \ph{TREND_BOLT_KEEP_S10} & \ph{TREND_BOLT_KEEP_S30} & \ph{TREND_BOLT_KEEP_S50} & \ph{TREND_TF_KEEP_S10} & \ph{TREND_TF_KEEP_S30} & \ph{TREND_TF_KEEP_S50} & \ph{TREND_CH2_KEEP_S10} & \ph{TREND_CH2_KEEP_S30} & \ph{TREND_CH2_KEEP_S50} \\
    \textsc{Best Fixed} & \ph{TREND_BOLT_BF_S10} & \ph{TREND_BOLT_BF_S30} & \ph{TREND_BOLT_BF_S50} & \ph{TREND_TF_BF_S10} & \ph{TREND_TF_BF_S30} & \ph{TREND_TF_BF_S50} & \ph{TREND_CH2_BF_S10} & \ph{TREND_CH2_BF_S30} & \ph{TREND_CH2_BF_S50} \\
    R2-CART & \ph{TREND_BOLT_R2_S10} & \ph{TREND_BOLT_R2_S30} & \ph{TREND_BOLT_R2_S50} & \ph{TREND_TF_R2_S10} & \ph{TREND_TF_R2_S30} & \ph{TREND_TF_R2_S50} & \ph{TREND_CH2_R2_S10} & \ph{TREND_CH2_R2_S30} & \ph{TREND_CH2_R2_S50} \\
    Fixed SAITS & \ph{TREND_BOLT_SAITS_S10} & \ph{TREND_BOLT_SAITS_S30} & \ph{TREND_BOLT_SAITS_S50} & \ph{TREND_TF_SAITS_S10} & \ph{TREND_TF_SAITS_S30} & \ph{TREND_TF_SAITS_S50} & \ph{TREND_CH2_SAITS_S10} & \ph{TREND_CH2_SAITS_S30} & \ph{TREND_CH2_SAITS_S50} \\
    TATO & \ph{TREND_BOLT_TATO_S10} & \ph{TREND_BOLT_TATO_S30} & \ph{TREND_BOLT_TATO_S50} & \ph{TREND_TF_TATO_S10} & \ph{TREND_TF_TATO_S30} & \ph{TREND_TF_TATO_S50} & \ph{TREND_CH2_TATO_S10} & \ph{TREND_CH2_TATO_S30} & \ph{TREND_CH2_TATO_S50} \\
    \introact{} & \ph{TREND_BOLT_OURS_S10} & \ph{TREND_BOLT_OURS_S30} & \ph{TREND_BOLT_OURS_S50} & \ph{TREND_TF_OURS_S10} & \ph{TREND_TF_OURS_S30} & \ph{TREND_TF_OURS_S50} & \ph{TREND_CH2_OURS_S10} & \ph{TREND_CH2_OURS_S30} & \ph{TREND_CH2_OURS_S50} \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccc}
    ...
    \textsc{SAITS}         & \ph{REC_SAITS_MSE}  & \ph{REC_SAITS_MAE}  & \ph{REC_SAITS_UTIL}  & \ph{REC_SAITS_N} \\
    \midrule
...
reading the main tables. The table is descriptive and is not used to choose an action or tune
the selector. \ph{UTILITY_SEVERITY_READING}. \ph{UTILITY_GUARD_READING}.
...
\resizebox{\textwidth}{!}{\begin{tabular}{llcccc}
    ...
    \multirow{5}{*}{Bolt}
      ...
      & \textsc{SAITS}         & \ph{UB_BOLT_SAITS_ALL}  & \ph{UB_BOLT_SAITS_S10}  & \ph{UB_BOLT_SAITS_S30}  & \ph{UB_BOLT_SAITS_S50} \\
    \midrule
    \multirow{5}{*}{TimesFM}
      ...
      & \textsc{SAITS}         & \ph{UB_TF_SAITS_ALL}  & \ph{UB_TF_SAITS_S10}  & \ph{UB_TF_SAITS_S30}  & \ph{UB_TF_SAITS_S50} \\
    \midrule
    \multirow{5}{*}{Chronos-2}
      ...
      & \textsc{SAITS}         & \ph{UB_CH2_SAITS_ALL}  & \ph{UB_CH2_SAITS_S10}  & \ph{UB_CH2_SAITS_S30}  & \ph{UB_CH2_SAITS_S50} \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\begin{table}[h]
  ...
  The table inspects how both quantities move over the registered penalty grid.}
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    \midrule
    $\beta = 0$ & \ph{OPPT_BOLT_B00_IR} & \ph{OPPT_BOLT_B00_HIR} & \ph{OPPT_TF_B00_IR} & \ph{OPPT_TF_B00_HIR} & \ph{OPPT_CH2_B00_IR} & \ph{OPPT_CH2_B00_HIR} \\
    $\beta = 0.5$ & \ph{OPPT_BOLT_B05_IR} & \ph{OPPT_BOLT_B05_HIR} & \ph{OPPT_TF_B05_IR} & \ph{OPPT_TF_B05_HIR} & \ph{OPPT_CH2_B05_IR} & \ph{OPPT_CH2_B05_HIR} \\
    $\beta = 1$ & \ph{OPPT_BOLT_B10_IR} & \ph{OPPT_BOLT_B10_HIR} & \ph{OPPT_TF_B10_IR} & \ph{OPPT_TF_B10_HIR} & \ph{OPPT_CH2_B10_IR} & \ph{OPPT_CH2_B10_HIR} \\
    $\beta = 1.64$ & \ph{OPPT_BOLT_B164_IR} & \ph{OPPT_BOLT_B164_HIR} & \ph{OPPT_TF_B164_IR} & \ph{OPPT_TF_B164_HIR} & \ph{OPPT_CH2_B164_IR} & \ph{OPPT_CH2_B164_HIR} \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\begin{tabular}{lccc}
    ...
    \midrule
    1 (lowest score) & \ph{SU_B1_N} & \ph{SU_B1_MEAN} & \ph{SU_B1_CI} \\
    2 & \ph{SU_B2_N} & \ph{SU_B2_MEAN} & \ph{SU_B2_CI} \\
    3 & \ph{SU_B3_N} & \ph{SU_B3_MEAN} & \ph{SU_B3_CI} \\
    4 & \ph{SU_B4_N} & \ph{SU_B4_MEAN} & \ph{SU_B4_CI} \\
    5 (highest score) & \ph{SU_B5_N} & \ph{SU_B5_MEAN} & \ph{SU_B5_CI} \\
    \bottomrule
  \end{tabular}
\end{table}
...
The stability of the cross-validated surface is checked directly. This subsection repeats the
whole selection procedure on parent-level
...
\resizebox{\textwidth}{!}{\begin{tabular}{lccccc}
    ...
    \midrule
\multicolumn{6}{c}{\ph{SELSTAB_ROWS}} \\
    \bottomrule
  \end{tabular}}
\end{table}
...
Bolt and \ph{SEED_SPREAD_CART_CH2} on Chronos-2 over the same three realisations.
These comparisons assess sensitivity to the deletion pattern. Reporting the spread over three
...
\begin{table}[h]
  ...
  protocol-defined subset. The spread across realisations is a stability diagnostic and is not
...
\resizebox{\textwidth}{!}{\begin{tabular}{lcccccc}
    ...
    \midrule
    Fixed SAITS           & \ph{SAITS_OFF}     & \ph{SAITS_CAND}     & \ph{SAITS_CALLS}     & \ph{SAITS_MEAN}     & \ph{SAITS_P95}     & \ph{SAITS_MAX} \\
...
configuration identifier. Historical TEST results informed this revision; they must not be
described as unseen development evidence. The revised run keeps TEST records out of fitting,
but evaluation on the same parents remains post-hoc.
...
The TEST manifest is the authoritative record of which parents and origins belong to the reused
evaluation population. It is fixed before the v47 rerun, and every TEST-derived table is generated
against that manifest. Replay-bank diagnostics, TRAIN-eval checks, and cost measurements use
their separately recorded manifests.
