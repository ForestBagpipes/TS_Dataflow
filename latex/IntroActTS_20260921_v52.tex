...
selector. Pooling action utilities increases aggregate error, while removing intervention
features improves it under the frozen configuration; given its own training-side selection,
the reduced state degrades one backbone, so the full state is retained. Controls matched on
development intervention frequency show that the gain over random gating comes from which
requests are repaired rather than how many, and they expose an accuracy-harm tradeoff. The
margin over the stronger simple controls is concentrated in one source. These results
motivate selective repair and identify limits of the
current estimator.
...
evaluation connects action variability to forecast accuracy, then tests decision quality
and the role of action-specific retrieval across eight sources and three backbones. It
retains competitive simple controls, and records where the full rule's advantage
concentrates: broad-based against the unchanged input, but resting largely on one source
against the stronger controls. A reduced feature state stays competitive under the frozen
configuration and is not adopted once it is given its own training-side selection. The current design was informed
...
backbone's own: Chronos-Bolt and Chronos-2 accept NaN entries under their native mask
semantics, TimesFM-2.5's official input pipeline interpolates within the context and strips
NaN prefixes, and the adapter adds no imputation of its own and records a non-finite output
as a failure. Reference behaviour therefore
...
only a weak, nonmonotone relationship with estimation error. Local retrieval should therefore be assessed by its resulting decisions.
...
guaranteed on that branch, and a configuration for which it triggered should instead be
recorded as infeasible and served as \textsc{Keep}. The branch never triggered in any
recorded selection. The anchor, the cap, the admissible settings and the selected pair
are fixed for the recorded run. Earlier TEST outcomes informed the method design.
...
$40.2\%$ increase loss. Its harmful loss is 0.0375, compared with 0.0513 for Best Fixed and
0.0584 for R2-CART. Figure~\ref{fig:harm} retains these aggregate diagnostics in the appendix.

To assess whether the rule does more than reduce intervention frequency, we calibrate
mean-only, linear and random controls to the full method's development intervention rate,
then freeze their thresholds or probability. Figure~\ref{fig:gate-controls} reports actual
TEST rates alongside accuracy and harm. Random gating preserves the full rule's recommended
repair. The other controls can also change which repair is recommended.

The full rule improves MASE over random gating by 0.0292, 0.1182 and 0.0682 on Bolt,
TimesFM and Chronos-2. All three nominal intervals exclude zero. Its advantage over the
mean-only control excludes zero only on TimesFM, and over the linear control only on Bolt.
Random gating nevertheless has lower harmful loss on all three backbones. The result
supports improved accuracy over random intervention at development-matched rates, without
establishing uniformly lower harm or superiority over the alternative utility controls.
Appendix~\ref{app:v52} gives exact values and the limitations of these post-hoc comparisons.

Two further controls match the comparison partners themselves on frequency. The simple
selector's acceptance threshold is recalibrated on the development block to the full
method's development intervention rate and then frozen; a random-KEEP variant of Best Fixed
executes the fixed repair with a per-request hash probability calibrated the same way. The
threshold-matched simple selector reaches 1.404, 1.590 and 1.412 source-macro MASE; the
paired difference against the full rule excludes zero only on Bolt ($-0.0252$,
$[-0.0495,-0.0011]$). The random-KEEP fixed policy is worse than the full rule on all three
...
rematched. Intervening less often therefore does not by itself reproduce the full rule's
accuracy, while the margin over the frequency-matched simple selector remains
backbone-dependent. Appendix~\ref{app:v53} records both controls.
...
A leave-one-source-out check recomputes the source-macro differences on each seven-source
...
broad-based. Appendix~\ref{app:v53} reports all eight rows per backbone.

A related diagnostic asks whether the gain comes from identifying which repair suits a
source rather than which repair suits a request. A source-level fixed policy applies, per
source, the single action with the highest mean realised utility on the replay bank, with
\textsc{Keep} as the registered fallback where that action is unsupported, all fixed on
TRAIN. It reaches 1.360, 1.527 and 1.448 source-macro MASE: better than the full rule on
Bolt ($+0.0181$, $[0.0061,0.0296]$) and TimesFM ($+0.0269$, $[0.0035,0.0526]$), worse on
Chronos-2 ($-0.0704$, interval includes zero), and close in aggregate (1.445 against
1.437). Request-level selection therefore does not outperform a per-source fixed choice on
two of the three backbones; the evidence for it rests on the broad-based margin over the
unchanged input and on the frequency-matched controls of \S\ref{sec:exp-harm}.
...
Table~\ref{tab:ablation} tests how the selector uses historical utility. A1 uses action-wise
global means. The revised A2 pools local neighbours across repair actions and retains only
global action-specific priors. It uses the full method's frozen $(k,\beta)$ without retuning.
A3 removes reference-forecast features, A4 always repairs, and A5 predicts utility linearly.

Pooling action utilities increases MASE from 1.4366 to 1.4470 and intervention frequency from
$65.4\%$ to $82.6\%$. The paired difference is $-0.0252$ on Bolt, with a nominal $95\%$
interval of $[-0.0478,-0.0025]$. The intervals include zero on TimesFM and Chronos-2.
This comparison supports action-specific local estimation within the tested design, while
also changing the balance between local evidence and global priors.

The original A2 removed only intervention features and remains a separate feature ablation.
It improves aggregate MASE to 1.4185, with the Bolt interval favouring this reduced state.
The linear estimator reaches 1.4343. Thus, neither the full feature set nor an accuracy
advantage over linear prediction is established. Appendix~\ref{app:v52} retains both A2
definitions, per-backbone intervals and additional recorded blocks.

That feature ablation reuses the full method's frozen $(k,\beta)$. As a final selection
check, the reduced 17-dimensional state was also given its own TRAIN-side selection under
the identical grid, cap and cross-validation. It then picks weaker penalties
($(k,\beta)=(64,0.5)$ on Bolt, $(16,0.5)$ on TimesFM and $(64,0.5)$ on Chronos-2, against
the full state's $(128,0.5)$, $(32,1.64)$ and $(16,0.5)$). On the development block the
retuned reduced state degrades TimesFM (MASE $1.0952 \to 1.1182$, paired interval
$[0.0012,0.0581]$ for the increase, harmful loss $0.0111 \to 0.0351$, intervention rate
$46.7\% \to 81.6\%$), leaves Bolt's accuracy unchanged while raising its harmful loss
($0.0279 \to 0.0408$), and improves Chronos-2 ($1.1365 \to 1.1131$, interval
$[0.0035,0.0477]$ for the decrease). Because the evidence is mixed, with a significant
single-backbone degradation and higher harmful loss on two backbones, the full
22-dimensional state is retained; the TEST improvement of the frozen-setting variant did
not transfer to the retuned one (1.4280 against 1.4366 aggregate, recorded for the record
only). Four of the five intervention features are identically zero under the present
catalog (Appendix~\ref{app:features}), so this comparison effectively removes one varying
feature together with the selection it induces. Appendix~\ref{app:v53} records the full
selection and evaluation tables.

Always acting raises harmful loss from 0.0375 to 0.0533. The full method averages 1.65
forecasting-backbone calls per request. Candidate construction is an additional cost paid
before selection, as specified in Appendix~\ref{app:calls}.
...
\adjustbox{max width=\textwidth}{\begin{tabular}{lccccc}
    ...
    A2 Pooled action utilities & 1.447 & 82.6\% & 44.4\% & 0.0478 & 1.83 \\
...
than the fixed repair and simple selector. Frequency-matched controls indicate that the
accuracy gain over random intervention comes from which requests are repaired, not how
many. The margin over the stronger controls is concentrated in one source, and a per-source
fixed policy is at least as accurate on two of the three backbones. Action pooling weakens
aggregate accuracy, while a reduced feature state stays competitive under the frozen
configuration and is not adopted once given its own selection. Development-matched
random gating has higher error but lower harmful loss. These findings bound the evidence
for the full decision rule. Independent evaluation remains
...
\adjustbox{max width=\textwidth}{\begin{tabular}{@{}p{.18\textwidth}p{.44\textwidth}p{.31\textwidth}@{}}
    ...
    Additional controls (unnumbered) & mechanism and feature controls, gate controls, the reduced-state selection check, source-level fixed policy and leave-one-source-out & which checks were run after the first review cycle, and what do they bound? \\
...
effect of this convention is retained as an open item in Appendix~\ref{app:v52}. The recorded
...
\clearpage
\section{Numerical Values for Action Utility}
...
\clearpage
\section{Additional Mechanism and Gate Controls}
\label{app:v52}
These controls were run after reviewing the earlier TEST results. They use the retained
prediction cache and frozen full-method settings. They do not constitute an independent
confirmation set. All intervals in this section are the recorded 10,000-resample paired
parent-bootstrap intervals and are nominal, without multiple-comparison adjustment.

The mechanism control pools neighbours across all repair actions, with global action-wise
utility priors. The original feature control removes the five intervention features while
retaining same-action retrieval. These answer different questions. Neither result justifies
relabeling the feature control as a mechanism ablation or suppressing its favourable error.
...
effect of the intervention-feature convention is exercised by the reduced-state selection
check of Appendix~\ref{app:v53}. These controls do not
certify the separate implementation verification effort or remeasure end-to-end latency.

\clearpage
\section{Reduced-State Selection and Source-Level Diagnostics}
\label{app:v53}
The checks in this section were run after the controls of Appendix~\ref{app:v52}, on the
same retained prediction cache, with selection confined to the TRAIN side (no TEST record
is read during selection, which the loader asserts). All TEST numbers remain post-hoc
comparisons on the previously examined block. Intervals are the recorded 2,000-resample
paired parent-bootstrap intervals (seed 101), nominal and without multiple-comparison
adjustment.

\subsection{The Reduced State Given Its Own Selection}
\label{app:v53-compact}
The feature ablation A2 of \S\ref{sec:exp-ablation} removes the five intervention features
...
\label{app:v53-gates}
Table~\ref{tab:v53-gates} extends the gate controls of Appendix~\ref{app:v52} from generic
