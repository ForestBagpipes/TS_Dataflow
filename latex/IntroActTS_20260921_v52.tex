...
features improves it. Controls matched on development intervention frequency also expose an
accuracy-harm tradeoff. These results motivate selective repair and identify limits of the
current estimator.
...
retains competitive simple controls and a feature ablation that improves on the full method. The current design was informed
...
than the fixed repair and simple selector. Action pooling weakens aggregate accuracy, while
a reduced feature state and a linear estimator remain competitive. Development-matched
...
\begin{table}[htbp]
  ...
  recorded dual-horizon subset. The spread across realisations is a stability diagnostic and is not
...
\adjustbox{max width=\textwidth}{\begin{tabular}{llccc}
    \toprule
    Backbone & Mask realisation & MASE $\downarrow$ & $\Delta$ vs \textsc{Keep} $\downarrow$ & Intervention rate \\
    \midrule
...
Independent confirmation, parent-aggregated retrieval and the run-level effect of the
intervention-feature missing-value convention remain unresolved. These controls do not
