...
| paper | arXiv | first public | status |
|---|---|---|---|
...
The first three predate submission by five, four and nearly four months. They
...
**No shared platform exists.** These four operate on mobile GUI action traces,
...
So these four are separated conceptually, not numerically, and the section says
so rather than leaving the absence unexplained.

## The single distinguishing sentence

These works assume the risk signal is a reasonable proxy for harm, and they
calibrate a signal taken to be reliable. We show that in data curation the most
