...
DEPLOY = ("NATIVE_KEEP", "BEST_FIXED", "R2_CART", "FIXED_SAITS", "TATO", "FULL_INTROACT")
DISPLAY = {"NATIVE_KEEP": "the untouched input", "BEST_FIXED": "the best fixed intervention",
           "R2_CART": "the simple selector", "FIXED_SAITS": "the fixed imputer", "TATO": "TATO",
...
def build(evals: dict, primary: dict, mean_over, ci,
    ...
    rivals = [r for r in ("NATIVE_KEEP", "BEST_FIXED", "R2_CART", "FIXED_SAITS", "TATO")
    ...
    published = ", ".join(f"{DISPLAY[name]} {m(name):.3f}" for name in ("FIXED_SAITS", "TATO")
    ...
    for name in ("FIXED_SAITS", "TATO"):
        if m(name) is not None:
            spread[DISPLAY[name]] = m(name)
    ...
    out["MAIN_BASELINE_SPREAD"] = (
        ...
        f"{oracle:.3f}, so what the catalog can recover is bounded and every row sits inside "
        f"that bound")
    published = {name: m(name) for name in ("FIXED_SAITS", "TATO") if m(name) is not None}
    ...
    deployable_rows = ("NATIVE_KEEP", "BEST_FIXED", "R2_CART", "FIXED_SAITS", "TATO",
                       "FULL_INTROACT")
    ...
    always = [name for name in ("FIXED_SAITS", "TATO")
              if m(name) is not None]
    tail = ""
    if always:
        tail = (", while the two rows that always repair act on every request their repair is "
                "available on")
    out["HARM_READING"] = (
        f"{INTRO} executes an intervention on {ir * 100:.0f}{PCT} of the requests, so its "
        f"gain is not the gain from standing still{tail}")
    out["HARM_IR"] = (
    ...
    others = ", ".join(f"{DISPLAY.get(name, name)} {m(name, 'harmful_loss'):.4f}" for name in
                       ("BEST_FIXED", "R2_CART", "FIXED_SAITS", "TATO")
    ...
    others_hir = [m(name, "conditional_hir") for name in
                  ("BEST_FIXED", "R2_CART", "FIXED_SAITS", "TATO")
                  if m(name, "conditional_hir") is not None]
    close = others_hir and (max(others_hir) - hir) < 0.05
    out["HARM_HL"] = (
        "What the rule lowers is how often an action runs and what an error costs when one "
        "happens. The share of executed actions that raise the loss stays close to what the "
        "fixed policies reach, so that share is a property of the catalog rather than of how "
        "cautious the rule is"
        if close else
        "The selective rule executes fewer actions, a smaller share of them raise the loss, and "
        "the ones that do cost less")
    out["HARM_BP"] = (
    ...
    if a["A2_WO_INTERVENTION"]:
        ...
        out["ABL_A2"] = (
            ...
            + (", so on that backbone the block costs more than it returns" if against_us else ""))
    if a["A3_WO_FORECAST"]:
        out["ABL_A3"] = (
    ...
    if a["A5_PARAMETRIC_RIDGE"]:
        out["ABL_A5"] = (
            ...
            f"costs {ci(a['A5_PARAMETRIC_RIDGE'])}, the largest gap in the table"
            + (", so the local estimate is what carries the result"
               if a['A5_PARAMETRIC_RIDGE']["excludes_zero"] else
               ", though the interval covers zero"))
    ...
    out["RANK_DISAGREE_STATS"] = (
        ...
        f"{best[1]['p_positive'] * 100:.0f}{PCT}, so no single repair serves every request and "
        f"none can be dropped from the catalog")
    ...
    if "chronos2" in evals:
        ch2 = evals["chronos2"]
        rows_ch2 = ch2["rows"]
        ch2_best = min(("NATIVE_KEEP", "BEST_FIXED", "R2_CART", "FIXED_SAITS", "TATO",
                        "FULL_INTROACT"),
    ...
    out["CONCL_MAIN"] = (
        ...
        f"{concl_tail}")
    a4 = comp.get("FULL_INTROACT_vs_A4_ALWAYS_ACT")
    a4_costly = bool(a4 and a4["excludes_zero"] and a4["difference"] > 0)
    out["CONCL_HARM"] = (
        f"The reference action is returned on {(1 - ir) * 100:.0f}{PCT} of requests, which lowers "
        f"both the call count and the harmful loss"
        + (", at a measurable cost in accuracy on the primary backbone" if a4_costly
           else " and is not separated on accuracy from acting every time"))
    if "chronos2" in evals:
        # What the held-out family shows is whatever it shows, so the sentence is
        # assembled from the two facts rather than fixed in advance.
        lowest = ch2_best == "FULL_INTROACT"
        first = ch2_rank_best[0] == "FULL_INTROACT"
        if lowest and first:
            held = ("reaches the lowest macro average and the best average rank of the "
                    "deployable rows there")
        elif first:
            held = (f"has the best average rank of the deployable rows there, while "
                    f"{DISPLAY.get(ch2_best, ch2_best)} reaches a lower macro average")
        elif lowest:
            held = (f"reaches the lowest macro average there, while "
                    f"{DISPLAY.get(ch2_rank_best[0], ch2_rank_best[0])} has the better "
                    f"average rank")
        else:
            held = (f"is behind {DISPLAY.get(ch2_best, ch2_best)} on both the macro average "
                    f"and the rank there")
        out["CONCL_TRANSFER"] = (
            "The same procedure, with both hyperparameters chosen by the same rule on that "
            "backbone's own replay bank, improves on the untouched input and " + held +
            ", on a family that took no part in method design")
    ...
    out["ABSTRACT_CLOSING"] = (
        "For a frozen forecaster the useful question about an incomplete input is which repair to "
        "run on this request, and a record of what past repairs did to this same forecaster "
        "answers it")
    out["CONCLUSION_CLOSING"] = (
        "A better imputer helps on some requests and hurts on others, and a frozen forecaster "
        "needs a record that tells the two apart")
    return out
