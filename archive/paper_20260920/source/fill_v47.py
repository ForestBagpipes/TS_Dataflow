"""Replace the \\ph{KEY} placeholders with the numbers the v4.7 run produced.

Reads ``results/v47/evaluation/test_<backbone>.json`` for whatever backbones
have finished and ``results/v47/protocol/selection_<backbone>.json`` for the
...
Usage:  python latex/fill_v47.py <source.tex> <target.tex>
"""
...
def load() -> tuple[dict, dict]:
    ...
    for backbone, _tag in BACKBONES:
        ...
        path = ROOT / f"results/v47/diagnostics/reconstruction_test_{backbone}.json"
        if path.exists() and backbone in evals:
            evals[backbone]["reconstruction"] = json.loads(path.read_text())
    return evals, selections


#: Every key that averages over backbones needs all of them.  A partial run
#: would otherwise print one backbone's value in a column the caption calls a
#: three-backbone average, and the sentence around it would read as if the
#: whole matrix were in.  Missing backbones leave those keys as placeholders.
REQUIRED_BACKBONES = {"bolt", "timesfm", "chronos2"}


def complete(evals: dict) -> bool:
    return REQUIRED_BACKBONES <= set(evals)


def mean_over(evals: dict, method: str, field: str):
    if not complete(evals):
        return None
...
def build(evals: dict, selections: dict) -> dict:
    ...
    # The reading sentences quote the aggregate, so they wait for it too.
    if complete(evals):
        out.update(fill_v47_reading.build(evals, primary, mean_over, ci, selections))
    # These modules read every backbone's records and average over them, so a
    # partial run would write one family's value into a column whose caption
    # calls it an average.  They wait; the environment keys do not average and
    # do not have to.
    if complete(evals):
        import fill_v47_extra
        out.update(fill_v47_extra.build(evals, selections, num, pct))
        import fill_v47_cost
        out.update(fill_v47_cost.build(num, pct))
        import fill_v47_strata
        out.update(fill_v47_strata.build(num, pct))
        import fill_v47_new
        out.update(fill_v47_new.build(num, pct))
    import fill_v47_env
    out.update(fill_v47_env.build())
    return {k: v for k, v in out.items() if v is not None}


def main() -> None:
    src = Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "latex/IntroActTS_20260919_v47.tex"
    dst = Path(sys.argv[2]) if len(sys.argv) > 2 else ROOT / "latex/IntroActTS_20260919_v47_filled.tex"
    ...
    print(json.dumps({"backbones": sorted(evals), "filled": before - after,
                      ...
                      "complete": complete(evals),
                      "target": str(dst)}, indent=1))
    if not complete(evals):
        missing_backbones = sorted(REQUIRED_BACKBONES - set(evals))
        print(f"PARTIAL FILL: {', '.join(missing_backbones)} not on disk, so every "
              f"key that averages over backbones is still a placeholder. "
              f"This file is not the manuscript.")
    (ROOT / "results/v47/evaluation/placeholder_status.json").write_text(
