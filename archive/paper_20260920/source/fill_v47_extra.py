...
def build(evals: dict, selections: dict, num, pct) -> dict:
    ...
    for backbone in ("bolt", "timesfm", "chronos2"):
        path = ROOT / f"results/v47/ablations/banksize_test_{backbone}.json"
        if path.exists():
    ...
    for backbone in ("bolt", "timesfm", "chronos2"):
        path = ROOT / f"results/v47/ablations/selection_stability_test_{backbone}.json"
        if not path.exists():
            continue
        payload = json.loads(path.read_text())
        keep = None
        eval_path = ROOT / f"results/v47/evaluation/test_{backbone}.json"
        if eval_path.exists():
    ...
    for backbone in ("bolt", "timesfm", "chronos2"):
        path = ROOT / f"results/v47/diagnostics/score_utility_test_{backbone}.json"
        if not path.exists():
            continue
        payload = json.loads(path.read_text())
