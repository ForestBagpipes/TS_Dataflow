...
TITLE_SIZE, AXIS_SIZE, TICK_SIZE, VALUE_SIZE, LEGEND_SIZE = 11, 11, 10, 10, 11
plt.rcParams.update({
...
def read(name):
    with (FIG / 'data' / 'v56_layout' / name).open(encoding='utf8', newline='') as f:
...
def utility():
    ...
    paired_bars(ax, [float(v['best_share']) for v in r],
                ...
                labels, ACTION_COLORS, '(a) Action shares (%)')
    ax.set_ylim(0,65);ax.set_yticks([0,20,40,60])
    ax.set_title('(a) Action shares (%)',loc='left',weight='normal',pad=8)
...
def diagnostics():
    ...
    paired_bars(a,[float(r['best_share']) for r in utility_rows],
                ...
                action_labels,ACTION_COLORS,'(a) Action shares (%)')
