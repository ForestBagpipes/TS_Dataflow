...
source-macro MASE, the lowest of nine comparison rows, against 1.576 for the unchanged input
and 1.445 for the strongest fixed policy, and it does so while repairing $55.2\%$ of requests
and adding 0.0340 of harmful loss. Four published repair methods applied to every request all
land above the unchanged input, between 1.612 and 1.906, which is the clearest evidence in the
study that repairing without deciding costs accuracy. A control that keeps the same
recommendations and randomises only whether they run is worse on every backbone, so the gain
comes from choosing which requests to repair. Evaluated once on two datasets held out of the
present design cycle, the margin over the unchanged input reproduces. The gain concentrates
where requests inside a source disagree about which repair helps, and it disappears where one
repair dominates a source, which is where the method reduces to a per-source lookup.
...
calibrated. It evaluates the layer on eight sources and three frozen backbones against nine comparison
rows, measures where the gain comes from and where it stops, and repeats the evaluation once
on data held out of the design cycle.
...
Imputation reconstructs missing observations, whereas task-oriented imputation evaluates
repairs through downstream performance~\citep{du2023saits,wang2024taskoriented,hao2025toivsf}.
We select among repairs for an unchanged forecasting model. TATO~\citep{qiu2026tato} selects
one transformation pipeline per domain, while our decision varies by request. Selective
prediction and learning to defer control which predictions are served~\citep{geifman2017selective,mozannar2020consistent}.
Here, keeping the input still returns a forecast from the same model. Historical replay
provides the outcomes of all admissible actions, making their forecasting utility available
for supervision. Appendix~\ref{sec:related} gives the full comparison.
...
Let $X \in \mathbb{R}^{C \times L}$ be a context with $C$ channels and length $L$, with
observation mask $M \in \{0,1\}^{C \times L}$ and an unavailable future target
$Y \in \mathbb{R}^{C \times H}$. The frozen pipeline $F$ carries the backbone's own
preprocessing and returns $p = F(X)$, so the reference action, which passes the incomplete
context through unchanged, inherits whatever that backbone does with gaps. The \textsc{Keep}
rows of Table~\ref{tab:main} show how much that differs across backbones, and
Appendix~\ref{app:method-details} specifies each adapter.

The system may modify the input only through a fixed catalog
$\calA = \{a_0, a_1, \dots, a_J\}$, where each action materialises an admissible version
$X^{a} = T_a(X, M)$, the reference $a_0$ returns the input unchanged, and
$\calA^{+} = \calA \setminus \{a_0\}$ holds the rest. Let $\loss$ be the episode-level MASE,
which is also the primary evaluation metric, so replay utilities and reported errors share one
scale. The forecasting utility of action $a$ is the reference loss minus the repaired loss,
\begin{equation}
...
Four constraints define the deployment protocol, each for a reason. A valid observation that
has already arrived may not be overwritten, so every $X^{a}$ differs from $X^{a_0}$ only where
$M = 0$. A repair may write only inside the range of the visible target channel widened by
three robust scales, because a candidate outside it is an extrapolation the protocol declines
to serve, and such a candidate is recorded as unsupported instead of executed. The realised
loss is unobservable at decision time, which is what makes the choice a prediction problem
rather than a lookup. The reference forecast is the only forecast that may be computed before
the decision, so a request spends one backbone call to obtain $p_0 = F(X^{a_0})$ and the
forecasts of the remaining candidates stay unseen while the action is chosen.
...
groups that answer different questions. The first applies one member of that catalog to every
request, asking whether the choice has to be made per request at all, and holds the unchanged
input, the single repair with the best TRAIN utility, the per-source fixed choice and fixed
SAITS. The second chooses per request by a simpler rule, R2-CART over the same state
features, asking what the pooled estimator adds beyond any per-request selection. The third
replaces the catalog with a stronger repair run on every request, holding TATO, which
searches a transformation pipeline per domain, and the trained imputers TimesNet, PSW-I and
T1, asking whether a better imputer removes the need to decide.
...
Earlier evaluations on this TEST block informed the current design, so the comparisons made
on it are exploratory. The confirmatory sources of \S\ref{sec:exp-main} were registered on a
fixed date and withheld from every decision in the present cycle, so they test a frozen rule
on data it was not fitted to, which is weaker than a wholly unseen source and stronger than
reading this block again.

\subsection{Variation in Repair Utility}
\label{sec:exp-exists}
Both the preferred repair and the value of repairing vary by request. Oracle-best shares in
Figure~\ref{fig:utility}(a,b) run from $11.1\%$ to $24.8\%$, KEEP is best on $19.3\%$ of
episodes, and reading each episode with its best action gives 1.258 MASE against 1.576 for
KEEP, which is the opportunity a deployable selector competes for. Reconstruction quality
does not reach it: on Bolt the two winners agree on $19.3\%$ of episodes, the rankings
disagree on $44.5\%$ of ordered pairs, and the mean within-episode Spearman correlation is
0.12. Selection therefore has to be supervised by the forecasting outcome itself, which is
what the replay bank records. Appendix~\ref{app:recutils} reports the rankings and
Figure~\ref{fig:cases} two retrospectively selected examples.

\begin{figure}[t]
\centering
\includegraphics[width=0.93\textwidth]{figure/fig8_diagnostics.eps}
...
missingness and every mean averages three backbones, two horizons and four patterns with equal
source weight. Table~\ref{tab:heterogeneity} keeps the action values and
Appendices~\ref{app:governance-fig} and \ref{app:severity-full} the breakdowns.}
...
\label{sec:exp-main}
Table~\ref{tab:main} compares the methods at $10\%$ missingness. \introact{} reaches 1.425
source-macro MASE and no other row reaches 1.44. Leaving the input unchanged gives 1.576, the
best single repair chosen on TRAIN gives 1.488, a per-source fixed choice gives 1.445, and the
two per-request controls give 1.459. The method gets there while repairing $55.2\%$ of
requests, which is less often than any row that repairs at all.

The four published repair methods sit on the other side of the unchanged input. TATO reaches
1.616, PSW-I 1.612, TimesNet 1.741 and T1 1.906, and each of them is worse than doing nothing
on every backbone. Each runs its published configuration on the same TRAIN region and the
same masks, so the gap is not a configuration artefact. What they share is that they repair
every request, and a repair that rescues a gap-heavy context still has to be paid for on a
context the backbone was already handling. The paired differences against them are the largest
in the table and exclude zero on every backbone for PSW-I and T1.
...
Holm adjustment. Against the best single repair the interval excludes zero on Bolt
($-0.0454$) and crosses it on TimesFM ($-0.0502$) and Chronos-2 ($-0.0915$) once the Holm
adjustment is applied across the nine comparison rows. Against the per-source fixed choice the
three backbones split: Bolt favours the source policy by $+0.0223$ with an interval of
$[+0.0044,+0.0432]$, TimesFM is a tie at $+0.0017$, and Chronos-2 favours per-request
selection by $-0.0834$ with an interval that crosses zero. Section~\ref{sec:exp-ablation}
shows that this split follows the cross-validated pooling strength, which is zero on Bolt and
Chronos-2 and 64 on TimesFM, so the comparison with a per-source policy is decided by how much
within-source signal each bank actually carries.
...
Earlier revisions read this block, so those comparisons are post-hoc. Two further datasets
were held back from the present design cycle and evaluated once with everything frozen. Solar
and US Term Structure got their own replay bank, their own cross-validated configuration and
their own calibrated threshold, all from the code that ran on the main set, and the evaluation
ran a single time on their 21 test parents. \introact{} reaches 4.226 source-macro MASE there
against 4.707 for the unchanged input, with intervals excluding zero on Bolt ($-0.837$) and
Chronos-2 ($-0.309$) and covering zero on TimesFM, so the margin over the unchanged input
reproduces off the data that shaped the design. The margin over the best single repair does
not, since that row reaches 4.223 and the backbones disagree in sign, which 21 parents cannot
resolve. Appendix~\ref{app:confirmatory} gives the protocol and the full table.

Average error does not say which repairs helped and which harmed, which the next section
separates.

\subsection{Intervention Frequency and Harm}
\label{sec:exp-harm}
Harmful loss measures the error added by repairs that made a forecast worse, averaged over
all requests. Figure~\ref{fig:harm}(c,d) records 0.0340 for \introact{} at a $55.2\%$ repair
rate against 0.0513 at $74.5\%$ for Best Fixed, 0.0552 at $98.7\%$ for R2-CART and 0.1798 for
TimesNet, which repairs everything. Repairing less is part of that, and a random gate
separates the two: it keeps the full rule's recommendation and randomises only whether it
runs, with its probability matched on the internal block and then frozen. It is worse on every
backbone, by 0.0534 on Bolt, 0.0142 on TimesFM and 0.1126 on Chronos-2, with intervals
$[-0.0751,-0.0336]$, $[-0.0281,-0.0001]$ and $[-0.1818,-0.0448]$. The rule is choosing which
requests to repair and not merely repairing rarely. Realised TEST rates differ from the
matched ones, so these remain development-calibrated comparisons.
...
Chronos-2 cells and only at $\alpha=0.05$ on Bolt and TimesFM, because temporal blocks do
not establish the exchangeability the bound assumes.
...
their endpoint by construction. How much each bank carries is therefore what the pooling
strength reports, and it is the same quantity that decides the Source Fixed comparison in
\S\ref{sec:exp-main}.
...
The direction of that reversal is what the design predicts. Selectivity pays when a
substantial share of requests is better left alone, and that share shrinks as gaps widen: the
rule repairs $55.2\%$ of requests at $10\%$ severity, $64.0\%$ at $30\%$ and $71.9\%$ at
$50\%$. Once nearly every request needs repairing, deciding per request buys little over
picking one repair per source, and a deployment expecting that regime would recalibrate the
threshold of \S\ref{sec:exp-harm} at its own severity, which costs no retraining. All three
severities occur in replay construction, so the sweep measures behaviour inside the sampled
grid. Appendix~\ref{app:layout-details} retains the mask-sensitivity checks.



\subsection{Deployment Cost}
\label{sec:exp-cost}
Timed end to end on an idle card at batch one, a served request takes a median of 94 ms on
...
backbone, so a deployment that wants the decision without that price can drop those two
actions. Appendix~\ref{app:calls-measured} gives the stage breakdown and the cold start.

\section{Conclusion}
\label{sec:conclusion}
A serving system in front of a frozen forecaster can only change the input it passes in, and
the right change differs from request to request and cannot be read off reconstruction
quality. \introact{} makes that change a decision, supervised by the forecasting utility each
repair produced on historical windows, estimated by pooling a request's neighbourhood with
the record of the same repair on its source, and executed only above a calibrated threshold.
It gives the lowest source-macro error of nine comparison rows while repairing half the
requests, and the gain appears where requests inside a source disagree about which repair
helps.
...
makes the rule both less accurate and less selective. The trend is consistent with the
estimator's design, since a neighbourhood drawn from fewer parents carries a smaller
$n_{\mathrm{eff}}$ and the score leans further on the source level mean. Three points do not
...
The main evaluation block has been read by earlier revisions of this work, so its comparisons
cannot rule out that the design adapted to it. Two further sources carry a weaker but still
useful guarantee. Solar carries 137 channels at ten minute resolution and US Term Structure
carries 40 daily yield curve series with a native missing rate of about $4.3\%$ that the data
contract preserves. Both appear in an earlier generation of this work, under a protocol whose
splits, masks, catalog and state definition all differ from the present one, so the honest
claim is not that the model has never seen the sources. It is that the confirmatory grid was
registered on a fixed date, that nothing in the present design cycle read it, and that no
hyperparameter, threshold, figure or intermediate report in this paper was produced from it.
The run below therefore tests a frozen rule against data it was not fitted to, which is weaker
than a genuinely unseen source and stronger than reading the main block again.
...
\begin{table}[htbp]
  ...
  $10\%$ missingness over 21 test parents. Neither source took part in any design decision.
  The paired difference is \introact{} minus the row, so a negative value favours
  \introact{}, and an asterisk marks an interval that excludes zero after the Holm
  adjustment within the backbone.}
...
\adjustbox{max width=\textwidth}{\begin{tabular}{lccccccc}
    ...
    Best Fixed & 4.409 & 3.917 & 4.344 & \best{4.223} & $-0.137^{*}$ & $+0.275$ & $-0.129^{*}$ \\
    Source Fixed & 5.022 & \best{3.831} & 4.036 & 4.296 & $-0.750^{*}$ & $+0.361^{*}$ & $+0.178$ \\
    Fixed SAITS & 5.095 & 4.324 & 4.588 & 4.669 & $-0.824^{*}$ & $-0.133$ & $-0.373^{*}$ \\
    R2-CART & 4.870 & 4.279 & 4.172 & 4.440 & $-0.599$ & $-0.087$ & $+0.043$ \\
    \midrule
    \rowcolor{bestgray}\introact{} & \best{4.271} & 4.191 & \best{4.215} & \second{4.226} & -- & -- & -- \\
    \midrule
...
Two readings are supported. The improvement over the unchanged input transfers: it is the
largest margin in the table, it holds on two of the three backbones with intervals that
exclude zero, and it holds on data whose scale is three times the main set's, which is the
claim the paper makes first. The improvement over the best single repair does not transfer at
this sample size: the two rows are 4.226 and 4.223 apart in aggregate and the three backbones
disagree in sign, and 21 parents give the paired bootstrap too little to separate them. The
cross-validated pooling strength on this set is zero on Bolt and TimesFM and 64 on Chronos-2,
which is the same pattern the main set shows, so the confirmatory run does not contradict the
estimator analysis of \S\ref{sec:exp-ablation} either.

\section{Selection Rule}
\label{app:selection-rule}

The cross-validated error surface over $(k,\beta,\lambda)$ is flat. Of the settings the harm
cap admits, twelve on Bolt, twelve on TimesFM and thirty five on Chronos-2 lie within one
standard error of the lowest cross-validated source-macro error, where the standard error is
that of the paired difference against the leader on the same bank episodes. Taking the strict
minimum of a surface that flat reads noise as signal, and it does so in a direction that
matters here, because error falls slowly across the band while the intervention rate rises
quickly, so the minimum lands on a setting that repairs far more often than it needs to. The
reported configuration is therefore the least intervening setting inside the one standard
error band, with the smaller neighbourhood breaking a remaining tie.

The alternative reading is recorded rather than discarded. Under the strict minimum the frozen
settings are $(8, 1.0, 0)$ on Bolt, $(8, 1.64, 64)$ on TimesFM and $(128, 0.5, 4)$ on
Chronos-2, against $(256, 1.64, 0)$, $(32, 1.64, 4)$ and $(64, 1.64, 4)$ under the rule used
here. The full evaluation was run under both and both result sets are retained. The minimum
rule gives 1.431 source-macro MASE against 1.421, repairs $76.5\%$ of requests against
$54.4\%$, and adds 0.0430 of harmful loss against 0.0361, so the choice costs nothing in
accuracy and buys a substantially quieter rule. Every other number in the paper comes from the
one standard error configuration.
...
the frozen catalog passes on all eighty requests, so the timed path is the path the accuracy
tables describe.
...
\begin{table}[htbp]
  ...
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lrrrrrr@{}}
    ...
    \midrule
    \multicolumn{7}{l}{\emph{Mean stage time of \introact{}, in milliseconds}} \\
...
medians because a request whose repairs are all admissible materialises every candidate, while
a request with a complete target materialises none.
