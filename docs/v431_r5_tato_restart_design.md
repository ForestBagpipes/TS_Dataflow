...
1. **确定性从零重跑（采用）**：在新目录重新执行同一 500-trial 搜索，放宽单场景墙钟上限。旧 partial 目录保持只读，通过参数序列、已完成 TRAIN 分数和预测的字节/数值复现核对重跑前缀。优点是 Optuna 状态与输出账本完整，缺点是重复一部分 GPU 计算。
...
- 新搜索从 trial 0 开始，旧 partial 的全部参数前缀一致；旧 completed trial 的 TRAIN 分数一致；逐样本预测优先核对字节哈希，哈希不同时只允许 `max_abs / max(1, |old|, |new|) <= 1e-8` 的浮点数值复现，并记录不一致计数和最大误差。该界限覆盖重启后实际观测到的最多一个 float32 ULP（全前缀最大缩放差 `5.35e-9`），但仍拒绝可影响结论的实质预测变化；
...
2026-09-16 首次正式启动目录 `tato-scene-restart-20260916` 因队列误用 core 解释器、缺少 worker 所需的 `tqdm` 而在模型加载前失败，失败账本原样保留。修订后的 `tato-scene-restart-20260916-r2` 在队列预登记中绑定 `W2_CHRONOS_PY` 路径和解释器文件 SHA；运行器不得改用自身解释器。该修订只修复执行环境，不改变任何研究身份字段。
