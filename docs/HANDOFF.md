> 最终代码/结果提交 `13d0f99a7698f003ec41eda5df1ad35a71c1459c`，精简审阅包SHA256 `1f4d1d2bcec2322fcf0348a5ac8a868373674b1ce351bc716da94f7ce35cd128`；已完成全部登记计算的实际范围和失败见v431_r5_delivery/final_snapshot。包发布提交另由Git记录。
...
## 2026-09-16 04:28 CST：r5冻结负结果与完整三来源基线子表

r5自身未重训、未扩展配置，开发门槛失败且calibration/test封存。最新已推送checkpoint前缀5a6c3fe。两家族旧DEV主表、自然在线及费用结论保持；离线TF高预算另1次局部solver超时明确单列，未超过总预算不等于无失败。

TATO缩放单位12场已全部保存并独立审核部署，26parent/52变体子表：Bolt scene1.416821、免费/r5 1.131513、R2 1.114892；TimesFM scene1.221407、免费1.052925、r5 1.057130、R2 1.052183。部分TRAIN搜索到截止为partial，不能称完整官方复现，也不与旧156变体绝对MASE混比。官方96单位四场正在单独执行，长度不支持保留；该表不混入上述缩放适配。

Chronos-2原生KEEP已完成旧DEV156变体，MASE0.957177；104物理DEV调用、52合法复用，只为骨干敏感性。ETTm1并非更优，不能宣称逐来源普遍优势或r5贡献。其首个记录层失败、917.244秒下载和成功执行费用均保留。

8来源输入/mask、重叠和时间异常准备完成，主实验候选确认未启动。剩余时间仅准备已登记TRAIN来源的有限原生接口检查，不读取新未来标签、不据其表现改方法。最终状态与全部缺项见v431_r5_final_snapshot、main_readiness与next_prompt。重启后旧04:25/04:45截止不能直接复用。

## 2026-09-16 03:38 CST：基线进度与有限工程修复

两Bolt及TimesFM H96的TATO TRAIN场景各500trial完成；TimesFM H192在途，方法与r5结论不变。首个Chronos-2 TRAIN在一次原生调用后因记录字典labels_read重复而失败，6.313467秒及原始NPZ/哈希保留；仅修记录层并增加回归，相同模型/输入重试已排队，DEV未运行。

两家族已完成H96的TRAIN raw重复输出逐值相同（Bolt3575/14500、TimesFM3239/14500）。未来8个extra场景改用独立同parent缓存worker，原trial/时间/数据/模型/监督不变，原等待状态和请求全部归档；每次部署清空，查找与首次推断成本保留。真实GPU效果尚待运行，不宣称已加速。official96仍等待实际extra队列状态；04:25/04:45截止不变。

论文补齐收益凸外包、FW近似界和真实停止条件，生成同账本统计图。0.05秒为软时限，不是硬实时保证。修正在线页遗留待运行文字，实际88请求及原负结果不改写。新增回归只覆盖缓存、字典记录及等锁后截止。


## 2026-09-16 r5截止前基线与数据准备增量（03:06 CST）

r5首个交付81f9ccbc2f4e9e7102443d8c94f8d50e80901531已推送，方法准入失败不变。首个精简审阅包SHA256 a19732e23e7fef40219e488d1b058db8bf811da04ed02bf8cc70eea44eff5aeb，不含权重/数据/凭据。

新增TATO TRAIN场景搜索和官方96长度单位适配，现两Bolt场景各500trial完成；TimesFM及其余来源尚在队列，最终实际状态见v431_r5_tato_scene_results。16/32缩放单位不等官方96原生空间，不能称完整复现。Chronos-2权重已核验，原生检查仍待GPU；TimesFM-3只核官方元数据。8来源准备286parent，104元数据上暂无已知重叠/时间异常；未解封确认、未跑主矩阵模型。

服务器截止05:04:31 CST；extra队列04:25截止为official96留时，所有新重任务最迟04:45停止启动，04:55前归档。只替换无GPU子进程的自有等待队列，未动他人任务；等锁后预算复核回归通过。详细报告必须在聊天交付。
...
## r4已完成在线（2026-09-16 00:43）

两家族自然38请求及受控6请求已完成，数值预测复核通过，hash差异另存；Bolt自然1个3.5秒超预算保留，TimesFM0，金融各12请求均high0超支。初次GPU锁冲突失败留档，改独立队列锁后重试完成。CPU与GPU队列结束；不继续扩张算法或打开确认集。共同表、论文及审阅包从README进入，提交以review.json和Git远端核验为准。本机F盘通过codex-save-local后既有Windows快进同步，不声称已从服务器观察到落盘。
...
当前工作采用用户新授权的并行调度：CPU终态拟合、单GPU证据/近期基线/第二模型家族、完整agent与论文同步推进。旧记录中的“先H2/H3通过才扩强baseline或第二TSFM”保留为历史，不再作为当前启动条件；数据隔离、真实依赖、冻结后生成获取标签和确认门槛继续有效。

已完成43个本地策略及1个TATO官方实现短预算适配的Bolt对照。共同开发集为26个独立parent、156个变体、三个来源，固定TS-ICL MASE为1.157005；同证据普通CART为1.136486。新单步agent全部STOP，MASE为1.200158，未超过强固定或简单对照；不能把CART收益归为agent贡献，H2/H3尚无正向方法证据。TATO完整数值和预算限制以[当日共同报告](v431_sprint_report.md)的动态主表为准。TimesFM独立家族在本条同步时仍在统一GPU队列，未填预计成绩，实际终态以该报告及机器状态为准。

22项本轮针对性测试通过；独立复核覆盖6,656份新增历史输出、2,768份current原始预测、6,708条共同决策、8终态/2获取器/432条T_acq标签及费用。当前费用表使用 `results/v431/20260914-sprint/accounted_table.json`，补计816次dirty诊断，初始化/离线生成/在线端到端费用分列；原冻结结果、负结果和首次失败记录不覆盖。

calibration/test继续封存，`PICS_joint_relabel`保持原协议incumbent。本轮为开发结果，未晋升、不宣称SOTA或ICLR录用，不将RED写入DOCX已验证研究成果。配置、原始逐窗证据、代码归因、在线状态及缺项见[冲刺报告](v431_sprint_report.md)、[执行计划](v431_sprint_plan.md)和[论文草稿](paper_v431_draft.md)。
