...
原156变体共同表中的近期对照为TATO两家族8trial适配。TimesFM旧1248trial中的546长度失败追溯至trimmer seq_l×32超过历史320/416，失败保留。新增TATO场景搜索按TRAIN选择后冻结、再评估旧DEV，结果单列于v431_r5_tato_scene_results；场景支持29/22/3个TRAIN parent，不冒充500个训练实例。现适配使用Bolt16、TimesFM32的长度单位，官方CLI默认96，故即使完成500trial也不等于官方完整原生空间复现。官方96单位、L512约束的额外适配与失败另列。
