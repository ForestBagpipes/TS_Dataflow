...
## 2026-09-16 r5 TATO 基线补齐（工程交付，不新增方法版本）

- 保留原 `tato-scene-restart-20260916` 的 5 个解释器启动失败；原因是队列误用 core 环境且缺少 `tqdm`，失败发生在模型/GPU执行前。
- 以冻结 Chronos 解释器及文件哈希创建独立的 `tato-scene-restart-20260916-r2`，单 GPU 串行完成 5 个场景，每场 500/500 trials。没有复用失败目录或覆盖旧 partial。
- 独立审计后 12 个缩放场景与 4 个官方 96 单位场景全部通过；三来源共同表为 52/52 变体。重启前缀最大缩放预测差 `5.3477674568242645e-09`，低于预先写入审计器的 `1e-8` 相对阈值；不声明逐位一致。
- 场景 TATO MASE 更新为 Bolt `1.4168208282`、TimesFM `1.2223876203`，仍差于同窗 R2 CART `1.1148918025`/`1.0521830913`。r5 无晋升，PICS_joint_relabel 历史协议身份不变，calibration/test 不读取。
- 本批属于既有基线的失败恢复、费用补齐和可复核交付，不创建 r6、不改预测策略、不构成 SOTA 或独立确认。

## 2026-09-23 v55 候选，未晋升

v55 对 v54 的局部效用估计加入来源均值池化，λ 网格为 0/4/16/64/∞。实施偏差与风险门边界见 `docs/v55_deviation_20260923.md`。来源均值现与 Source Fixed 同为合法回放记录均值，但 λ=∞ 的完整决策在固定动作不合法时会换用另一合法动作，因此不能称两策略完全等同。λ=0 与 v54 的 424 条 train_eval 请求逐动作分数相同，最大绝对差 0，决策一致率 100%。

登记修正后的 TEST 是探索性结果：Bolt/TimesFM/Chronos-2 的 λ 为 0/64/0，Full MASE 为 1.382575/1.528443/1.364959。只有 TimesFM 选池化，相对局部差 −0.012457，95% CI [−0.038625,+0.013504]。Bolt 对 Source Fixed 显著较差；不依据宏平均单项晋升方法。Solar/USTS 确认 TEST 尚未一次性评估，外部基线和 E3 尚未收口。历史 incumbent 与旧失败记录不变。
