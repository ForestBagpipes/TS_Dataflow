...
PSW-I 八来源修复已通过合并内容检查：`train_eval` 424 条，`test`、`test30`、`test50` 各 760 条；主 TEST 129 条不满足合理性约束，修复仍留档且分母不变。已建立 2704 个基线预测候选。PSW-I 和 T1 的三骨干预测尚未完成，主表不可填这两行，也不能宣称胜过全部九个对照。

15:35 更新：T1 的 Electricity、Weather 已完成，八来源合并的 `train_eval` 为 424 条，`test`、`test30`、`test50` 各 760 条；观测不一致计数和隐藏值非有限计数均为零。主 TEST 有 66 条修复超出合理性约束，档案保留；2704 个 T1 预测候选已生成。两方法的三骨干预测和完整主表仍待 GPU 收尾后运行。

旧 E3 延迟报告记录了 60 处动作合法性不一致，均为旧卡拥堵时 `MULTI_TSICL` 显存不足；原收尾脚本还误用无模型依赖的 core 解释器。该报告不能用于论文。已排入隔离环境重测和逐请求审计；详见 D-06。
