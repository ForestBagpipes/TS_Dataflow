...
from introact_ts.v44.hashing import array_hash, require
...
def plausibility_bounds(reference: np.ndarray) -> tuple[float, float]:
    ...
    """
    from introact_ts.v44.state import robust_scale
...
def implausible_reason(filled: np.ndarray, reference: np.ndarray) -> str | None:
    ...
    if not np.array_equal(filled[observed], reference[observed]):
        return "candidate rewrote an observed value"
    if not np.isfinite(filled).all():
...
def apply_action(name: str, panel: np.ndarray, *, imputer: Imputer | None = None,
    ...
    def outcome(filled: np.ndarray, *, applicable: bool = True,
        ...
        return ActionOutcome(
            ...
            reason=reason,
            changed=bool(applicable and not np.array_equal(filled, reference_target, equal_nan=True)),
