...
from .grid import mask_seed_of

REPLAY = "results/v47_verified/replay"
...
def load_catalog(root: str | Path, block: str, backbone: str) -> list[EpisodeCatalog]:
    ...
    input_path = replay / "inputs" / f"{block}.npz"
    tsicl_path = replay / "tsicl" / f"{block}.npz"
    saits_path = replay / "saits" / f"{block}.npz"
    expected = status.get("input_dependencies")
    actual = {"inputs": _sha256(input_path), "tsicl": _sha256(tsicl_path),
              "saits": _sha256(saits_path)}
    if expected != actual:
        raise ValueError(f"forecast dependencies changed for {block}/{backbone}")
    ...
    with np.load(replay / "inputs" / f"{block}.npz", allow_pickle=False) as store, \
        ...
        stores = (store, tsicl, saits_store)
        ...
        for row in manifest["rows"]:
            ...
            if reference_prediction is None:
                raise ValueError(f"missing KEEP prediction for {key}; refusing denominator change")
            ...
            for action in P.ACTIONS:
                ...
                if applicable:
                    ...
                    if name in predictions.files:
                        ...
                        raise ValueError(f"missing prediction for applicable action {key}/{action}")
                entry = ActionEntry(
                ...
                if prediction is not None:
                    try:
                        ...
                        raise ValueError(f"metric failure for {key}/{action}: {exc}") from exc
