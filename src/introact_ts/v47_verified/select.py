"""v4.7 selector: vectorised same-action retrieval, leave-one-parent-out
...
def distance_matrices(bank: Bank, queries: Queries, *, lopo: bool,
                      ...
                      same_source: bool = False) -> dict[str, np.ndarray]:
    ...
    """
    out = {}
    # Fit preprocessing on the training fold, before computing any distance.
    folds = []
    if lopo:
        for parent in np.unique(queries.parent):
            rows = np.flatnonzero(queries.parent == parent)
            training = [b.Z[b.parent != parent] for b in bank.per.values()
                        if len(b.Z) and np.any(b.parent != parent)]
            if not training:
                folds.append((rows, None, None))
                continue
            training = np.vstack(training)
            scale = training.std(0)
            folds.append((rows, training.mean(0), np.where(scale > 1e-12, scale, 1.0)))
    else:
        folds = [(np.arange(len(queries.episode)), bank.mean, bank.scale)]
    for action in ACTIONS:
        b = bank.per[action]
        if len(b.g) == 0:
            out[action] = np.zeros((len(queries.episode), 0))
            continue
        D = np.full((len(queries.episode), len(b.g)), np.inf)
        for rows, mean, scale in folds:
            if mean is None:
                continue
            Q = (queries.Z[action][rows] - mean) / scale
            B = (b.Z - mean) / scale
            finite = np.isfinite(Q).all(1)
            Q = np.where(np.isfinite(Q), Q, 0.0)
            distances = np.sqrt(np.maximum(
                (Q ** 2).sum(1)[:, None] + (B ** 2).sum(1)[None, :] - 2.0 * Q @ B.T, 0.0))
        ...
        if same_source:
            D[queries.source[:, None] != b.source[None, :]] = np.inf
        out[action] = D
    return out
...
def paired_cluster_bootstrap(queries: Queries, mase_a: np.ndarray, mase_b: np.ndarray,
    ...
    if not by_parent or resamples < 1:
        raise ValueError("bootstrap requires paired finite observations and positive resamples")
    parent_cells = {}
    for key, indices in by_parent.items():
        cells = collections.defaultdict(list)
        for i in indices:
            cells[(int(queries.horizon[i]), float(queries.severity[i]))].append(float(diff[i]))
        parent_cells[key] = {cell: float(np.mean(values)) for cell, values in cells.items()}

    def macro(sample: list[tuple[tuple, int]]) -> float:
        cells = collections.defaultdict(lambda: collections.defaultdict(list))
        for key, _tag in sample:
            for cell, value in parent_cells[key].items():
                cells[cell][key[0]].append(value)
        return float(np.mean([np.mean([np.mean(v) for v in sources.values()])
                              for sources in cells.values()]))
...
def harm_cap(bank: Bank, queries: Queries) -> dict:
    ...
    lowest source-macro realised MASE on the bank (unsupported requests KEEP).
    Its conditional harmful rate is
    ...
    """
    losses = _fixed_macro_losses(queries, NONREF)
    anchor = min(losses, key=losses.get)
    means = {a: source_macro(queries, np.where(queries.legal[a], queries.utility[a], 0.0))
             for a in NONREF}
    u = queries.utility[anchor]
    usable = np.isfinite(u) & queries.legal[anchor]
    cap = float((u[usable] < 0).mean()) if usable.any() else 0.0
    return {"anchor": anchor, "cap": cap, "anchor_mean_utility": means[anchor],
            "anchor_applicable": int(usable.sum()), "action_mean_utility": means,
            "action_source_macro_mase": losses}
...
def select_hyperparameters(bank: Bank, queries: Queries, *, k_grid=K_GRID,
    ...
    if fallback:
        raise ValueError("no grid setting satisfies the registered harm cap; freeze refused")
    ...
    for r in feasible:
        r["gap_to_selected"] = source_macro(queries, r["mase"] - best["mase"])
        r["gap_se"] = None  # 1-SE selection removed; no mismatched aggregation is reported
    for r in rows:
        r.pop("mase", None)
...
def best_fixed_action(bank: Bank) -> str:
    queries = Queries(bank.catalogs, blocks=bank.blocks)
    losses = _fixed_macro_losses(queries, NONREF)
    return min(losses, key=losses.get)


def _fixed_macro_losses(queries: Queries, actions) -> dict[str, float]:
    losses = {}
    for action in actions:
        values = realised(queries, apply_fixed(queries, action))
        if not np.isfinite(values).all():
            raise ValueError("fixed-action evaluation requires finite losses for every request")
        losses[action] = source_macro(queries, values)
    return losses
