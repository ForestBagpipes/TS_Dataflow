...
for Time Series Foundation Models via Behavioral Introspection \\[4pt]
\large IntroSpect-TS：基于行为内省的时序基础模型零训练数据质量评估}

\begin{document}
\maketitle

\begin{abstract}
The quality of pretraining data fundamentally determines time series foundation model (TSFM) performance, yet the TSFM community lacks principled methods for assessing per-sample data quality. While the LLM community has developed sophisticated frameworks, these either cannot process numerical time series or require expensive auxiliary training. We introduce IntroSpect-TS, a training-free method that extracts data quality signals directly from a frozen TSFM's inference behavior. The key insight: a pretrained TSFM implicitly encodes knowledge of normal temporal patterns. IntroSpect-TS collects multi-signal behavioral traces during a single forward pass, then applies profile-conditioned peer calibration to isolate quality-induced deviations from domain-induced difficulty. The method's design rests on a formal justification (Proposition~1) that makes explicit the assumptions required for valid quality identification. Experiments across three TSFM architectures and seven benchmarks aim to demonstrate that IntroSpect-TS enables better data curation than heuristic, learned, and raw-loss baselines, with ablation confirming peer calibration as the critical component. The method requires no parameter updates, no LLM calls, and no auxiliary training.

\vspace{4pt}
\noindent \textbf{摘要：}预训练数据质量从根本上决定时序基础模型（TSFM）的性能，然而TSFM社区缺乏原则性的逐样本质量评估方法。本文提出IntroSpect-TS，一种零训练方法，通过行为内省直接从冻结TSFM的推理行为中提取质量信号。核心洞察：预训练TSFM隐式编码了正常时序模式的知识。方法在单次前向传播中收集多信号行为轨迹，然后应用画像条件同类校准将质量引起的偏差与领域难度隔离开来。方法的设计建立在一个形式化论证之上（命题1），明确了有效质量识别所需的假设。在三种TSFM架构和七个基准上的实验旨在证明IntroSpect-TS能比启发式、学习和原始损失基线实现更好的数据筛选。
\end{abstract}

\section{引言}

时序基础模型（TSFM）已迅速成为预测领域的主导范式。TimesFM、MOIRAI、Chronos、MOMENT和Timer等架构在跨能源、交通、天气、金融和医疗的大规模时序语料库上预训练。隐含假设很简单：更多数据产生更具泛化能力的模型。然而这种“可用即包含”范式忽视了一个根本问题：所有预训练数据都同等有用吗？实践中，大规模语料库包含大量噪声和冗余数据，训练此类数据浪费计算资源并可能降低性能。一个自然的应对是训练有监督的质量分类器。然而，TSFM预训练语料库通常缺乏质量标签——标注100B+时间点中极小部分都不可行。挑战因此在于是\emph{无}标注数据地评估质量，仅使用预训练模型已有的信号。
...
我们提出IntroSpect-TS，一种零训练行为内省方法。核心洞察：预训练TSFM已隐式学会区分正常模式和异常信号。阶段1在单次前向传播中收集多信号行为轨迹，阶段2通过画像条件同类校准解决领域混淆——该识别论证形式化为命题~\ref{prop:identification}。方法的设计借鉴后门调整准则~\citep{pearl2009causality}，其中画像作为调整集，其价值在于明确校准有效所需的假设。阶段3按质量分层语料库，阶段4通过下游微调验证。

本文贡献：（1）形式化TSFM零训练数据质量评估问题，提出该领域首个行为内省方法。（2）提供形式化论证（命题~\ref{prop:identification}）建立画像条件同类归一化隔离质量与领域难度的条件，配以鲁棒实用算法。（3）设计系统实验方案，覆盖三种TSFM架构和七个基准数据集，包含消融、受控难-vs-脏测试、以及启发式和学习质量评分基线的对比。
...
\textbf{IntroSpect-TS} & \textbf{通过行为内省进行零训练质量评估} & \textbf{---} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{关键挑战与IntroSpect-TS设计选择。}
...
\textbf{基线。}（1）随机；（2）原始损失；（3）SNR-based；（4）异常率-based；（5）LLM直接评分；（6）\textbf{有监督学习评分器}——XGBoost在画像特征$\mathbf{p}_i$上训练，使用TSFM微调结果（top vs bottom 25\% MSE）的二元标签，5折交叉验证；（7）全量数据。有监督学习基线直接检验核心声明：如果画像特征在监督训练下能匹配IntroSpect-TS性能，则行为信号无增量价值。

\textbf{实现。}$K = 50$，均匀融合权重。微调：AdamW，lr $10^{-5}$，10 epoch，单A100。

\subsection{主要结果}

表1呈现投影验证。预期模式：IntroSpect-TS Top-25\%优于Bottom-25\%和所有基线。有监督学习评分器预计优于统计启发式但不及IntroSpect-TS——确认TSFM行为信号包含静态画像特征所不具备的质量相关信息。
...
表1中有监督学习列包含在画像特征上训练的XGBoost分类器。如果IntroSpect-TS优于它，确认行为信号提供超越静态特征的增量信息。如果它匹配或超过IntroSpect-TS，核心声明将被证伪。

\subsection{区分“难”与“脏”}

构造两个受控测试集：\textbf{HBC}——2,000个干净汇率窗口（金融时序分布与传感器类领域显著不同，从领域差异产生高原始损失）；\textbf{EBD}——2,000个注入15\%高斯噪声的电力窗口（15\%经校准使原始损失与HBC可比）。额外评估缺失片段和传感器漂移噪声类型。带同类校准的IntroSpect-TS将它们清晰分离；移除校准将分离瓦解。
...
IGDS和IntroSpect-TS汇聚在“模型内部信号指导数据”的共同原则上，通过互补机制实现。IGDS需SAE训练；IntroSpect-TS使用原始行为实现零成本部署。结合两者是有前途的方向。
...
本文提出IntroSpect-TS，一种用于TSFM数据质量评估的零训练行为内省方法。方法将问题分解为信号提取、领域-质量解缠和因果验证，通过形式化论证（命题~\ref{prop:identification}）支持的原则性框架解决每个子问题。
