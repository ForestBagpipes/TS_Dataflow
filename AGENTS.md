...
完成一批经过检查、适合保存的项目修改后，先检查 `git status`，再运行 `codex-save-local "简要说明本批修改"`。该命令检查补丁格式、提交并推送；用户登录 Windows 后，`F:\work\Time-research\work2` 约30秒内以 fast-forward 同步。不得提交密钥、密码、数据集、模型权重或临时大文件。本机已跟踪修改未提交或分支分叉会暂停同步，不得强推或重置绕过。远端发布成功不等于已观察到 Windows 同步完成。
