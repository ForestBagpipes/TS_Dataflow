...
DEFAULT_TRIALS = 48
DEFAULT_WINDOWS = 8
#: v4.7 searches on the densified bank block, which holds the same TRAIN
#: parents as before at every registered severity.
SEARCH_BLOCK = "bankx"
...
def main() -> None:
    ...
    with (ROOT / f"locks/gpu-tato-{args.backbone}.lock").open("a") as lock:
        fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        # One lock per backbone.  Every call is a single series on a small
        # checkpoint, so the device is latency bound and this stage runs beside
        # the others.  The floor only rules out a genuinely full device.
