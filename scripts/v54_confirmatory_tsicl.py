#!/usr/bin/env python3
"""v54 confirmatory Stage B -- TS-ICL intervention stage (GPU).

A mirror of ``scripts/v47_tsicl.py`` bound to the confirmatory grid and
writing only under ``results/v54/confirmatory/``.  The frozen TS-ICL
checkpoint runs through the unmodified v4.3 worker adapter, so the
intervention semantics are exactly the ones the project already froze.
...
def main() -> None:
    ...
    status = {
        "stage": "v54-confirmatory-tsicl-B",
        "block": args.block,
        "status": "completed",
        "identity": identity,
        "episodes": len(specs),
        ...
        "source_hashes": {
            ...
            "src/introact_ts/v54/confirmatory.py":
                sha(root / "src/introact_ts/v54/confirmatory.py"),
            "scripts/v54_confirmatory_tsicl.py": sha(Path(__file__)),
        },
