...
def main() -> None:
    ...
    specs = PL.episode_specs(root, args.block, parent_limit=args.pilot_parents)
    if args.only_source:
    ...
    with (root / "locks/gpu.lock").open("a") as lock:
        ...
        try:
            for spec, _raw, masked in PL.iter_contexts(root, specs):
                key = spec.episode_id
                ...
                for action, mode in (("SINGLE_TSICL", "none"),
                    ...
                    try:
                        ...
                        value[observed] = target[observed]
    ...
    status = {
        ...
        "source_hashes": {
            ...
            "scripts/v47_tsicl_verified.py": sha(Path(__file__)),
    ...
    if failures:
        raise SystemExit(f"TS-ICL failures: {len(failures)}; inspect recorded status")
