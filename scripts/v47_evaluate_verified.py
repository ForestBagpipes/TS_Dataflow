...
def load_baseline(root: Path, method: str, block: str, backbone: str,
    ...
    path = OUT / f"baselines/{method}_{block}_{backbone}/records.json"
    if not path.exists():
        return None
    ...
    if not np.isfinite(mase).any():
        return None
    if not np.isfinite(mase).all():
        raise RuntimeError(f"Incomplete baseline: {path}")
...
def main() -> None:
    ...
    parser.add_argument("--block", default="train_eval", choices=["train_eval", "test", "test30", "test50", "test_m2", "test_m3"])
    ...
    if args.block != "train_eval" or any(b.strip().startswith("test") for b in args.bank_blocks.split(",")):
        raise SystemExit("Only TRAIN-eval permitted before the development gate")
    OUT = Path(args.root) / args.output_root
    C.REPLAY = args.output_root + "/replay"

    began = time.perf_counter()
    root = Path(args.root)
    freeze = OUT / f"protocol/selection_{args.backbone}.json"
    ...
    def mean_ranks(pool):
        values = np.stack([mase_of[name] for name in pool], axis=1)
        if not np.isfinite(values).all():
            raise RuntimeError("Non-finite result: cannot change ranking denominator")
        ranks = rankdata(values, method="average", axis=1)
        return {name: float(ranks[:, i].mean()) for i, name in enumerate(pool)}
    average_rank = mean_ranks(ranked)
    ablation_rank = mean_ranks(["FULL_INTROACT"] + list(ABLATIONS))

    payload = {
        "stage": "v47-verified-development-evaluation",
        "test_records_read": 0,
        "missing_baselines": [label for label in BASELINES.values() if label not in external],
