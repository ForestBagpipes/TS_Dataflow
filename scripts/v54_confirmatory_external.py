#!/usr/bin/env python3
"""v54 confirmatory BRITS/CSDI as catalog actions, on the v47_saits.py protocol.

A mirror of ``scripts/v54_external_imputers.py`` bound to the confirmatory
grid (``introact_ts.v54.confirmatory``) and writing only under
``results/v54/confirmatory/``.  The protocol is identical: bank records are
cross-fitted over two parent folds, evaluation blocks are imputed by the model
fitted on the whole bank, one repaired target channel per episode is spliced
back against the observation mask, and only finite repairs are written.  The
plausibility guard is recorded here and enforced at the forecast stage,
exactly as for SAITS.

Outputs go to ``results/v54/confirmatory/replay/external/<method>/<block>.npz``
under the key ``<episode>|<METHOD>``.
"""
...
#: Frozen configurations, identical to scripts/v54_external_imputers.py.
CONFIGS = {
...
def splice(method: str, Y_raw, imputed, rows, arrays, failures):
    ...
    for i, (episode, _parent, _panel) in enumerate(rows):
        ...
        if not np.isfinite(repaired).all():
            ...
            continue
        # The guard is recorded, not enforced: the forecast stage marks the
...
def main() -> None:
    ...
    out = ROOT / f"results/v54/confirmatory/replay/external/{method.lower()}"
    ...
    write(report_path, {
        "stage": "v54-confirmatory-external-imputers",
        "method": method,
        "config": config,
