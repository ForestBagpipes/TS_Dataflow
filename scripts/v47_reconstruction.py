...
def main() -> None:
    ...
    store = np.load(root / "results/v47/replay/inputs" / f"{args.block}.npz",
                    allow_pickle=False)
    ...
    for label, (method, _tag) in EXTERNAL.items():
        cand = root / f"results/v47/baselines/{method}/candidates.npz"
        recs = root / f"results/v47/baselines/{method}_{args.block}_{args.backbone}/records.json"
