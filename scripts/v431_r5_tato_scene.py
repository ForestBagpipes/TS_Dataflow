#!/usr/bin/env python3
"""TRAIN scene-level TATO search and frozen current-context deployment.
...
def execute(request,pilot_only):
 ...
 import torch
 r=json.loads(Path(request).read_text());assert sha(__file__)==r['worker_sha256'];assert sha(ROOT/'scripts/v431_r5_prepare_main.py')==r['adapter_sha256'];assert sha(r['inputs'])==r['inputs_sha256']
