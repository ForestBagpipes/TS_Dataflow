...
def load_baseline(root: Path, method: str, block: str, backbone: str,
    ...
    path = root / f"results/v47/baselines/{method}_{block}_{backbone}/records.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    root = Path(args.root)
    freeze = root / f"results/v47/protocol/selection_{args.backbone}.json"
