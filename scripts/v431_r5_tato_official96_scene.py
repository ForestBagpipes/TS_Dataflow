#!/usr/bin/env python3
"""Official 96-step TATO unit audit under legal L512; independent runner.
...
def prepare(args):
 ...
 request=dict(status='prepared_not_run',family=args.family,horizon=args.horizon,source='ETTm1',target_channel=0,target_field='HUFL',condition='target_block_10',context=512,
  ...
  worker_sha256=sha(__file__),adapter_sha256=sha(ROOT/'scripts/v431_r5_tato_official96_adapter.py'),reference_frozen_sha256=sha(frozen),
  supervision='r5 T_fit parents only; no gate/check/acq/calibration/test',
  protocol='official patch/data/model units96 and seq_l5..15; L512 restricted scene audit, >512 unsupported; not official L1440/top16/Pareto reproduction',patch_len=96,data_patch_len=96,model_patch_len=96,initial_trimmer_seq_l=7,unit_audit_not_performance_selected=True,
...
def execute(request,pilot_only):
 from v431_r5_tato_official96_adapter import execute_frozen_scene,initial_params
 import tato_adapter as adapter
 import optuna
 import worker as baseline
 import torch
 r=json.loads(Path(request).read_text());assert sha(__file__)==r['worker_sha256'];assert sha(ROOT/'scripts/v431_r5_tato_official96_adapter.py')==r['adapter_sha256'];assert sha(r['inputs'])==r['inputs_sha256']
 ...
 with gpu_lock():
  ...
  if pilot_only:
   for row in training[:r['pilot_requests']]:
    tick=time.perf_counter();model.cache.clear()
    try:
     p,detail=forecast(row,initial_params());item=dict(uid=row['uid'],status='completed',**detail,prediction_hash=arrsha(p))
  ...
  tuner=tuner_factory.build_optuna_tuner(enqueue_param_dicts=[initial_params()],mode='train',seed=101)
  distribution=tuner_factory.build_search_space(adapter.NAMES,patch_len=96)
  search_begin=time.perf_counter()
  for n in range(r['trials']):
   if time.perf_counter()>=search_deadline:break
   trial=tuner.pick_trial(distribution);entry=dict(trial=n,params=dict(trial.params),samples=[]);tick=time.perf_counter();model.cache.clear();train_predictions={}
   try:
    errors=[]
    for row in training:
